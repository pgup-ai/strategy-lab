...
SOL was spent once, and it failed.** `scripts/r7d/` is the whole diff — **no `src/` change
...
**4 · The holdout, spent once.** SOL/USDT perp 4h, 12,914 bars, whole frame:
...
Against the declared threshold: beats R5 trained ✔, beats `donchian` ✘, survives 3× costs
✘ → **FAIL**. The plan's SOL derivation was ambiguous between "its own bars" and a training
cut; both were run and the verdict is identical (−39.47% / −0.4996 under the other reading).
...
2. **The selected cell is negative in sample**, so §5's deflation prices a search among
   losing rules.
3. **SOL's threshold is derived on its own whole frame**, a mild in-sample element that the
   plan's wording left ambiguous. The alternative reading is reported above and agrees.

...
| **R10h** | **Running the paper process** — the delayed oracle performed rather than simulated | A live window's bars, signals and per-bar reasons equal a replay of the same range once the venue has been re-fetched for it | R10 | — **next.** Designed at [docs/plans/2026-08-08-r10h-running-the-paper-process.md](../plans/2026-08-08-r10h-running-the-paper-process.md). R10 shipped `LiveFeed` and R10g shipped the book and **nothing ran either** — no CLI command constructed one — so R10's gate was passed against a scripted venue and the `db`-marked venue test its plan promised does not exist. Preparing to run found M45: **no perp poll had ever carried funding**, because a 5-bar window holds two settlements where the coverage guard needs three. |

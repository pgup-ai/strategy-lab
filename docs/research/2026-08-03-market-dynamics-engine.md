...
price columns are `NUMERIC(38,18)`. Measured: a price of 1.234567890123456e-05 loses digits
there, a BTC-scale price does not. CLAUDE.md's existing rule fixed the float8 → numeric
**cast**; the **scale** is a separate limit nobody had measured, and it bites any asset
priced below roughly 0.1. Recorded as M34, not repaired — `market_candles` is read-only in
this phase and repairing it is a migration with its own controls.
...
| M34 | **A `NUMERIC` scale counts decimal places where a float64 needs significant digits, so `NUMERIC(38,18)` silently truncates any value below ~0.1** | Found by R10a's diff landing on its own reading 2 ([§9.10](#910-r10a-the-why-layer--census-item-a-measured)): four features that should have agreed bit-for-bit disagreed on 3–28 bars of 1,000 by exactly one ULP, because a 1/480 rolling percentile is 0.0020833333333333333 — **19 decimal places** — and 18 is all the scale keeps. Unconstrained `NUMERIC` round-trips it; `bar_reasons` uses that and a parametrized test pins it. **The consequence reaches further than this phase and is deliberately not repaired here.** `market_candles`' price columns are `NUMERIC(38,18)`, and measured directly, a price of 1.234567890123456e-05 loses digits there while a BTC-scale price does not. CLAUDE.md's existing precision rule fixed the float8 → numeric **cast** — `col::text::numeric`, and `Decimal(str(float(x)))` on every write — and that rule is still right; the **scale** is a second, independent limit that no test covers and that bites any asset priced below roughly 0.1. The repo has never stored one, which is why it has never shown. Repairing it is a migration with its own controls, on a table this phase treats as read-only. | 2026-08-06 |

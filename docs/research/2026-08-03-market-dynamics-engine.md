...
| Date | Entry |
|---|---|
| 2026-08-06 | **R10b: the board ships, and it slices the single view rather than recomputing it.** `GET /api/board` streams one row per (dataset, strategy) as each finishes — state, feature values, latest fill, sparkline, provenance — and the page opens on a grid of tiles that link into the existing instrument view. **All six acceptance checks pass**, verified independently: rows are **identical** to `/api/analysis` over each row's own window (M36), a cached request issues **0 SQL statements**, `browse` issues **0 mutating statements**, and first paint is **67 ms** cold-cache / 28 ms warm against a declared bar of one second. **One of the plan's two options turned out not to exist**: parallelism returned **1.10×** on four threads, because the work is pandas and vectorbt under the GIL, so streaming is the only lever and that constrains every later attempt to widen the board. **One tradeoff is deliberate and visible**: bounding each frame at its last settlement leaves a perp row up to one bar stale (ETH `as_of` 16:00 against a 20:00 stored bar), shown in orange rather than hidden, because the alternative blanks every tile the moment a candle fetch outruns funding (M37). Perps only — census item (f) makes equity freshness a different problem and it is still unmeasured. See [§9.11](#911-r10b-the-board--several-instruments-one-computation). |
...
| 3 · cached request reads no candle | **pass** — **0 SQL statements** on the second request |
| 4 · a new bar invalidates exactly its rows | **pass** — key is `(identity, strategy, exit_mode)`, stamped on `dataset_last_bar` + the funding window |
| 5 · `browse` writes nothing | **pass** — 0 mutating statements, `signals`/`bar_reasons`/`market_candles` counts unchanged |
| 6 · first paint under a second | **pass** — first row **67 ms**, all four **1,156 ms** cold-cache; **28 ms / 64 ms** fully cached |
...
1. **The cache stamp cannot see a settlement rewritten in place** between the funding
   span's own endpoints. A re-fetch of the same range does not produce that; a corrected
   history would.

...
| **R7b** | **The state vector's missing axis** — gate entries on `energy` as well as `strength` | The diagnostic first: the composite must beat **each** single-feature gate by ≥ 5 pp on R7's own rate metric, both halves — and if it does not, the P&L is not read. Then one run on a **held-out instrument** | R7 | **Done — the diagnostic failed and the P&L was never read, which is the pre-registration working.** 0/4 on BTC and 0/4 on ETH against a 5 pp bar declared before the data was seen; the selection, the three evaluations and the deflated Sharpe never ran, and **the SOL holdout is unspent** (M28). The failure inverts the hypothesis rather than refuting it: `energy` adds to `strength` and `strength` does not add to `energy`, and alone at H=30 `energy ≤ 0.50` is positive in all four instrument-halves where `strength ≥ 0.80` is negative in three (M27). The code change ships anyway — `energy_ceiling` defaults to 1.0 and every published v1/v2 figure is bit-identical to `main` — so the axis is available to whatever comes next without a second migration. See [§9.7](#97-r7b-the-energy-gate--btcusdt-and-ethusdt-perp-4h). *Superseded content, from before it ran:* Pre-registered at [docs/plans/2026-08-06-r7b-energy-gate.md](../plans/2026-08-06-r7b-energy-gate.md) before any figure. R7 measured the entry gate's second term admitting 82% of bars and adding **+0.00 pp**, while `energy` — the axis §2.1 defines chop with — is the one feature carrying chop signal and is not read by the machine. So one predicate is added to `advancing`, `energy_ceiling` defaults to 1.0 so every published figure survives by construction, and **only that one parameter is selected** (M22 holds the R5 cell fixed). **SOL/USDT perp 4h is fetched as the holdout** — 12,914 bars, zero funding gaps, no split, touched once — because R7 read `energy`'s IC on BTC's test half and on ETH, which leaves neither able to answer this. |
| **R7c** | **The energy-first lifecycle** — invert the machine onto the axis that measures positive | A **tradeability** kill switch before any return (the selected cell must trade within 3× of R5's 77 round trips), then one run on the **held-out instrument**, which this phase spends | R7b | **Done — stopped at the kill switch, holdout unspent.** Every constructible cell fired the tradeability band (250–385 round trips against 26–231), so the SOL run, the overlay control and the DSR never happened. **The grid was mis-specified and the defect is the plan's**: declared in `energy` units against a gate declared in `strength` rank units, its tightest cell advances on 37.9% of bars where `strength ≥ 0.80` advances on 21.1%, and a matched threshold sits 56% below the declared floor (M29). **The hypothesis is therefore untested at matched selectivity**, the grid was not extended after the fact, and the re-run needs a fresh pre-registration — which still has a clean SOL to spend (M30). See [§9.8](#98-r7c-the-energy-first-lifecycle--stopped-at-the-kill-switch). *Superseded content, from before it ran:* Pre-registered at [docs/plans/2026-08-06-r7c-energy-first-lifecycle.md](../plans/2026-08-06-r7c-energy-first-lifecycle.md) before any figure. R7b settled the entry-gate question, so R7c does **not** re-ask it: `strength` drives both `advancing` and `failing`, so the hysteresis, the dwell and the whole six-state lifecycle sit on the axis that measures negative, and this moves both sides onto `energy` while `direction` keeps deciding side. Two things declared up front. **BTC and ETH are burned** — R7b read `energy`'s lift on both test halves, so their R7c numbers are reported and are not evidence — and **SOL is spent here**, which is why the kill switch sits before it. And an **overlay control** runs first, because "advance only when energy is low" may be nothing but *trade in low volatility*, which is a known effect and not a state machine. |
| **R7d** | **The lift at matched selectivity** — redo R7c with the grid declared in **coverage** | **Two kill switches, cheapest first**: does `energy`'s trend-rate lift survive tightening to R5's ~21% coverage (positive in all four instrument-halves, mean ≥ +3.0 pp), then R7c's tradeability band — and only then SOL | R7c | **Done — both switches cleared, the holdout was spent, and it failed.** The first switch cleared *upward*: `energy`'s lift **grew** when tightened to R5's selectivity (mean +6.67 / +6.15 / +6.73 pp against +4.93 loose, all four instrument-halves positive at every target), so the chop signal is real and not an artifact of a permissive ceiling. The second cleared because M29's fix worked — 107/159/223 round trips against R7c's 250–385 (M32). **Then SOL: −39.58% / −0.4800 / −90.22% at 3× costs**, DSR 0.1416 against 0.95. Outcome row 4: the estimator works and the machine cannot trade it (M31). See [§9.9](#99-r7d-the-lift-at-matched-selectivity--the-holdout-spent-and-failed). *Superseded content, from before it ran:* Pre-registered at [docs/plans/2026-08-06-r7d-matched-selectivity.md](../plans/2026-08-06-r7d-matched-selectivity.md) before any figure. **M29 applied as the design rule**: the grid is declared as enter coverage {15%, 21%, 30%} with exit at 2× that, and each frame derives its own `energy` value mechanically from the target — coverage is the portable quantity, since declaring the value would carry BTC's volatility distribution onto another instrument (M18). Three cells, not six. The phase exists because **the one positive measurement in four phases has never been tested at selectivity**: R7 measured it by tercile and R7b at 0.50/0.65/0.80, all far looser than the gate it replaces. **SOL is still clean** because R7c's switch fired before spending it (M30). |
| R8 | HMM / Kalman | Beats R5 rules out-of-sample by enough to justify the complexity | R7d | — **does not start.** M31: four phases have separated measuring state from earning from it, and R7d is the one that could have rescued the line and did the opposite by clearing its own diagnostics. The chop estimator **works** at matched selectivity on two instruments; the machine built on it loses in sample and on a clean holdout. A better estimator answers a question this program has now measured four times and found is not the one it is failing. Reopening this needs a new instrument (SOL is spent) **and** a reason to think execution changed. *Superseded content:* , and R7b narrowed why.** M25 blocked this on its inputs; R7b tested the obvious widening and the composite gate failed, but measured that the axis the machine *does* gate on gives a **negative** trend-rate lift in three of four instrument-halves while the one it ignores is positive in all four (M27). So the inputs problem is not "add a feature" but **which axis the lifecycle is built around**, and an estimator over the current four would be estimating a state vector gated on its weakest. Unblocking it now means a machine whose entry condition is energy-first, pre-registered with its own kill switch (M28) — and SOL is still a clean holdout for it. *Superseded content:* , not on its model (M25).** R7 measured the four features this would estimate over carrying **no** chop information on either instrument, while the axis §2.1 defines chop with — `energy` — is not among them. A better estimator of a state vector missing its chop axis is a better estimator of the wrong thing, so **the model progression is unchanged and does not start here**. What unblocks it is an inputs phase: widen the machine's feature set toward the nine dimensions §2.1 declared, `energy` first, and re-run R7's own diagnostic against the same declared bars before any model is fitted. |
...
| **R10e** | **The event path runs every strategy and every contract** — (b)'s `ExitMode`, (d)'s runner, M40's check | R10d's three measured gaps driven to **0**: `trend_following_deepseek_v4` 7,331/7,012, `turnaround_v1`/`v2` 984/1,035; `state_machine_v2` running on the event path; and a contract mismatch refused at construction | R10d | **Done — all six acceptance checks pass.** The runner takes an `ExitMode` and emits what it implies **by calling `engine._exit_signals` over the buffer** rather than reimplementing it (M36 on a third path), so R10d's gaps go to **0** across 18 (strategy, frame) pairs and 5,201 signals. `ExposureRunner` ships and `tests/test_exposure_determinism.py` **drives it** instead of its local stand-in. A contract mismatch is refused at construction with no bars fed (M40 closed). 41 stat keys bit-identical against a clean `main` worktree; 1,022 tests. **(c) closed without code**, and the reasoning is the finding: recomputable at 0 differing, already rendered by the browser, and a `Signal.size` would hand 7 of 9 strategies a field they can only leave `None`. `setup_invalidation_stop` is **refused** — the engine applies it as an intrabar `sl_stop` no bar-close signal encodes — and `trend_structure` can begin raising mid-run, which is the engine's behaviour surfaced at the first bar it is knowable (M41 records what a `side` means on a level). See [§9.14](#914-r10e-the-event-path-runs-every-strategy-and-every-contract). *Superseded content, from before it ran:* Designed at [docs/plans/2026-08-07-r10e-event-path-completeness.md](../plans/2026-08-07-r10e-event-path-completeness.md). **Deliberately before (a)**, on R10d's ordering: (a) touches `Bar`/`BarBuffer`/the feed/the schema and `state_machine_v2` reads crowding, so closing it first means re-threading the funding plumbing through the exposure path when it arrives — structurally complete first, then correct once. Nothing here can move a published number and (a) can, and **(e) cannot verify (a) until (a) lands**, so that check belongs to the same phase as that fix. Reconnaissance shrank two items: (d)'s storage half **already exists** from R6 (`ExposureSignal`, `write_signals`, the `target_exposure` column), so only the runner is missing; and **(c) closes as "no work needed"** — recomputable at 0 differing over 600 bars, already rendered by the browser, and a `Signal.size` would give 7 of 9 strategies a field they can only leave `None`. The exposure runner is **separate** rather than a widened `StrategyRunner`, the same argument the third registry rests on, and `tests/test_exposure_determinism.py`'s local driver is pointed at it — which is what makes it proven rather than merely written. |
| **R10f** | **Funding on the event path** — census items (a) and (e) | R10a's diff driven to zero on every feature and on the state, and a determinism suite that can *fail* on the difference it was blind to | R10e | **Done — all six acceptance checks pass, and the census is closed.** `Bar` carries `funding_rate`, `BarBuffer` materialises it, and `ReplayFeed.from_database` attaches it through the engine's own `with_funding_column`. R10a's diff: `crowding` **6,048 of 6,048 → 0**, state **488 → 0**. **(e) closed by mutation** — the determinism suite moved onto funded frames and a new test proves it can now fail on a crowding difference. **A column's presence is a claim** (M42): the buffer materialises `funding_rate` only when a bar really carried one, since `build_feature_frame` reads `FUNDING_COLUMN in df.columns` and an always-present NaN column would report `crowding_measured=True` on spot. Coverage is required only when the strategy reads funding — the same question `sweep` asks — so BTC's permanent 40 h leading gap stays replayable for `donchian`. Three strategies × 41 stat keys bit-identical against clean `main`; 1,029 tests. See [§9.15](#915-r10f-funding-on-the-event-path--census-items-a-and-e-closed). Designed at [docs/plans/2026-08-07-r10f-funding-on-the-event-path.md](../plans/2026-08-07-r10f-funding-on-the-event-path.md). |
| **R10g** | **Signals into positions — the book** | The book reproduces the backtest's own `trades.csv`, trade for trade, including a reversal — fed the engine-resolved `SignalSet` rather than driven through `ReplayFeed`, since that those two agree is `tests/test_replay_determinism.py`'s claim and re-testing it here would test that suite instead | R10f | **Done — all six checks pass.** `donchian` **19/19** and `turnaround_v1` **355/355** closed trades reproduce `trades.csv` on entry bar, exit bar, direction, size and both fill prices; `state_machine_v2` **12/12** orders match `from_orders`. The execution semantics are derived from the installed vectorbt, not chosen. **The oracle found what reasoning did not** (M43): sizing is non-compounding in the *request* and clipped by cash in the *fill*, `min(requested, balance / (price × (1 + fee)))`, caught as one wrong size in nineteen. 1,043 tests; published figures bit-identical. See [§9.16](#916-r10g-the-book--signals-into-positions-gated-on-tradescsv). *Superseded content, from before it ran:* Designed at [docs/plans/2026-08-08-r10g-the-book.md](../plans/2026-08-08-r10g-the-book.md). The census closed the *signal* gap and left the one `_extract`'s docstring has named since Phase 1a: signals are recorded, not driving positions, and there is **no book in `src/`**. Built before the live feed deliberately, because **the book has an oracle and the feed does not** — 53 stored `trades.csv` files, against which a book is correct in the only sense that matters. The execution semantics are *derived* rather than assumed, measured against the installed vectorbt: size off the raw close, buy fills at `close × (1 + slippage)` and sells at `(1 − slippage)`, fee on the filled notional, repeated same-side entries ignored, and **a short entry while long closes the long and opens the short — two trades on one bar, both at the sell price**, which is the rule a book would get wrong by guessing. |
| R10 | Paper trading | Research and live code paths produce identical output | R9 | — *superseded by the R10a–R10g and R10 rows above; kept for the original gate wording.* **next, and now the program's main line rather than a later phase.** M31 ended the state-machine sequence on the finding that the estimator works and the book does not, which makes execution the open problem and this the phase that owns it. Nothing blocks it: the read-only browser shipped the viewing half in PR #14, and the six ways a live signal differs from a backtested one are already enumerated as a work list in the 2026-08-05 state-of-play entry. *Original gate note:* , and the gap is now enumerated**: six ways a live signal differs from a backtested one, in the 2026-08-05 state-of-play entry. Two are structural rather than bugs — the runner withholds exit ingredients until Phase 1b gives it an `ExitMode`, and `TargetExposure` strategies cannot run on the event path at all. The product vision (a command-and-control dashboard) lands here, not earlier. |
| **R10** | **Paper trading — the live feed** | A live window's bars *and* signals equal a replay of the same window once it is stored: the delayed oracle, because a live feed has none at the moment it runs | R10g | **Done — all six acceptance checks pass.** Designed at [docs/plans/2026-08-08-r10-the-live-feed.md](../plans/2026-08-08-r10-the-live-feed.md). **It polls rather than opening a socket**, and that is a decision: the protocol does not care where a `BarEvent` came from, the venue REST client already exists and is tested, and at 4h and 1w the difference is not observable — `browse` makes the same argument for its own refresh. A websocket is an optimisation for a bar size nothing here trades, and it can land behind the same protocol later without touching the runner, the book or any strategy. Two §12 follow-ups stop being cosmetic here: `backfill()`/`prime()` do not compose, so a live process cannot warm itself from history — fatal when `state_machine_v1` emits nothing for 2,192 bars — and `Subscription.include_forming` is read nowhere, which a poll landing mid-bar makes load-bearing. |
| **R10h** | **Running the paper process** — the delayed oracle performed rather than simulated | A live window's bars, signals and per-bar reasons equal a replay of the same range once the venue has been re-fetched for it | R10 | **Done — five of six acceptance checks pass, and the sixth is a finding.** The delayed oracle reads **0 differing** on OHLCV and funding across the logged live bars, and **0 on the state and on every feature** (`crowding`, `direction`, `energy`, `stability`, `strength`) across 5 and 3 bars; the signals section is **not exercised**, because over 48 minutes at 15m neither path emits. Check 5 asked for 0 withheld polls and got **27 and 26** — coverage lapsed one cadence past the 16:00 settlement and the process could not clear it, which is M47. Designed at [docs/plans/2026-08-08-r10h-running-the-paper-process.md](../plans/2026-08-08-r10h-running-the-paper-process.md). R10 shipped `LiveFeed` and R10g shipped the book and **nothing ran either** — no CLI command constructed one — so R10's gate was passed against a scripted venue and the `db`-marked venue test its plan promised does not exist. Preparing to run found M45: **no perp poll had ever carried funding**, because a 5-bar window holds two settlements where the coverage guard needs three. |
| R11 | Canary (small size) | Expected vs actual fills, costs, state transitions all match | R10h | — |
...
| Date | Entry |
|---|---|
| 2026-08-08 | **R10: the live feed, checked against a delayed oracle.** `MarketDataFeed` had one implementation from Phase 1a until now, and it read from Postgres. **The phase's problem is that a live feed cannot be diffed against anything as it runs** — which is why R10g built the book first, against a frozen `trades.csv`. Its oracle is delayed: a live bar and a stored bar for the same interval are the same fact recorded twice, so a live window must equal a replay of that window, at bar level *and* at signal level. Both hold. **It polls rather than opening a socket**, deliberately — the protocol does not care where a `BarEvent` came from, the REST client already exists and is tested, and at 4h and 1w the difference is not observable, which is the argument `browse` already makes for its own refresh. Two §12 follow-ups stopped being cosmetic and are closed: `backfill()` and `prime()` now compose through `prime_bars(bars)`, without which a live process could not warm itself and `state_machine_v1` would emit nothing for a year at 4h; and `Subscription.include_forming` finally decides something. **The most useful result is about a test rather than the code** (M44): the supersede check passed while a first-wins mutant in `BarBuffer.append` survived, because the scripted venue returned the *same row* for an interval's forming and closed copies — a fixture too clean to distinguish the two states it was asserting over. Perturbing the forming row kills the mutant. 1,056 tests, published figures bit-identical, no network in the suite. See [§9.17](#917-r10-the-live-feed--a-delayed-oracle-for-the-one-path-that-has-none). |
| 2026-08-08 | **R10g: signals become positions, and the book agrees with the backtest trade for trade.** The gap `_extract`'s docstring has named since Phase 1a — signals recorded, nothing consuming them — closed by `engine/book.py` and `engine/exposure_book.py`, with the pricing rules in `engine/fills.py` **derived from the installed vectorbt** rather than chosen. Built before the live feed deliberately, because **the book has an oracle and the feed does not**: `trades.csv` is frozen, 53 of them are stored, and a book that reproduces one is correct in the only sense available. Gate met on stored BTC perp 4h — `donchian` **19/19** and `turnaround_v1` **355/355** closed trades matching on entry bar, exit bar, direction, size and both fill prices, with 2 and 322 reversals in range respectively; `state_machine_v2` **12/12** orders against `from_orders`. **The oracle found what reasoning did not** (M43): the first run differed on exactly one size, because sizing is non-compounding in the *request* and **clipped by available cash in the fill** — `min(requested, balance / (price × (1 + fee)))`, measured at 9,158.96 filled of 9,500 requested. A doubled account still asks for the original size; a halved one gets less than it asks for, and a book implementing only the first half diverges permanently after the first drawdown. 1,043 tests, published figures bit-identical. See [§9.16](#916-r10g-the-book--signals-into-positions-gated-on-tradescsv). |
| 2026-08-07 | **R10f: funding reaches the event path, and the census is closed.** The last and largest item. `Bar` carries `funding_rate: Decimal | None`, `BarBuffer` materialises it, and `ReplayFeed.from_database` attaches it through **`with_funding_column` -- the engine's own function** -- so the alignment rule and the coverage guard are shared rather than duplicated. Measured with R10a's own harness: `crowding` differing **6,048 of 6,048 -> 0**, the state **488 -> 0**, every other feature still 0. **Census item (e) closed too, and by mutation rather than assertion**: the determinism suite now runs on funded frames, and a new test strips funding from the streamed side and asserts the signals move -- a funded frame is only an improvement if the suite can *fail* on the difference it was blind to. **The load-bearing decision is that a column's presence is a claim** (M42): the buffer materialises `funding_rate` only when a bar really carried one, because `build_feature_frame` reads `FUNDING_COLUMN in df.columns` and an always-present NaN column would report `crowding_measured=True` on a spot frame -- replacing a fallback that is correct off-perp with a silent wrong answer. Two corollaries: a stream that stops settling **raises** rather than silently narrowing, and **coverage is required only when the strategy reads funding**, the same question `sweep` asks, so BTC's permanent 40 h leading gap stays replayable for `donchian`. Controls: three strategies x 41 stat keys bit-identical against a clean `main` worktree, 1,029 tests. **Two of R10a's own controls now report FAIL, and that is them working** -- they assert the live path ran crowding at the neutral constant, which is false by design now; CONTROL B is what keeps the zero honest, with 1,308 of 1,308 bars away from 0.5 on both sides agreeing to a max delta of 0. See [§9.15](#915-r10f-funding-on-the-event-path--census-items-a-and-e-closed). |
| 2026-08-07 | **R10e: the event path now runs every strategy and every contract, and census items (b) and (d) are closed.** A design plan whose acceptance checks were numbers R10d had already measured, which is what made it checkable rather than aspirational. **(b)**: `StrategyRunner` takes an `ExitMode` and emits the exits it implies **by calling `engine._exit_signals` over the buffer** -- the engine's function, not a reimplementation, which is M36 reaching a third path. R10d's gaps go to **0** across 18 (strategy, frame) pairs on BTC and ETH perp 4h, 5,201 emitted signals compared, with the six pairs already at 0 as the non-vacuity guard. Two refusals are deliberate: `setup_invalidation_stop` is rejected at construction because the engine applies it as an intrabar `sl_stop` that no bar-close `Signal` encodes, and `trend_structure` can begin raising **mid-run** because a growing buffer can acquire the short entries it refuses -- the engine's own behaviour, surfaced at the first bar it is knowable. **(d)**: `ExposureRunner` ships, and `tests/test_exposure_determinism.py` **drives it** rather than the local stand-in its header had been asking for, so all three of its comparisons now test the class a live process would run. **M40 is closed**: a contract mismatch raises at construction with *no bars fed*, where before it survived 2,192 bars. **(c) closed with no code at all**, and the reasoning is the result -- recomputable at 0 differing over 600 bars, already rendered by the browser, and a `Signal.size` would hand 7 of 9 strategies a field they can only ever leave `None`, which is the argument `ExposureSignal` exists to make. M41 records what a `side` means when it names a level rather than an event: a flip through zero is **one** row, because the level is the whole statement. Controls: 41 stat keys bit-identical against a clean `main` worktree on a real run, 1,022 tests (was 1,003), nothing crowding-aware and no published figure moved. One caveat is about a test of mine rather than the code -- "nothing is submitted through warmup" is false at a zero band, in the engine too. See [§9.14](#914-r10e-the-event-path-runs-every-strategy-and-every-contract). |
...
| 2026-08-06 | **R10a: the why-layer is stored where it cannot be recomputed, and census item (a) is now a number rather than a claim — the whole number.** A design plan (`81d02a2`) rather than a pre-registration, since it ships a schema and a diff. **The obvious version was half wrong**: the research path already computes a `WhyLayer` per bar and is right not to store it, so the event path persists, the research path recomputes, and R10's gate becomes the **diff between them** (M35). Measured on 6,048 BTC/USDT perp 4h bars: **`crowding` differs on 6,048 of 6,048**, `direction`/`energy`/`stability`/`strength` on **none**, the state on **488 (8.1%)** — and the live path spends 64% more bars in `RIDING` (848 against 515). Declared reading 1, and its unfalsifiable-looking clause was measured: **the research features with `crowding` pinned to the live constant reproduce all 6,048 stored states, 0 residual**, so (a) is the *entire* live/research gap for this strategy. **It hit reading 2 first, on the harness's own storage**: `NUMERIC(38,18)` truncates a 1/480 rolling percentile because a scale counts decimal *places* where a float64 needs *significant digits* — which also means `market_candles`' price columns lose digits for a price needing more than 18 fractional places — a small magnitude *and* full float64 precision together — recorded as **M34** and deliberately not repaired here. Controls: all four published v1/v2 rows bit-identical to a clean `main` worktree, 957 tests (was 928). Census items (b), (c), (d), (f) remain unmeasured. See [§9.10](#910-r10a-the-why-layer--census-item-a-measured). |
| 2026-08-06 | **R7d cleared both kill switches, spent the holdout, and failed it — which settles the state-machine line.** Pre-registered at `94e2969` with the grid declared in **coverage** rather than in feature units (M29), each frame deriving its own `energy` value from the target. **The first switch cleared and the lift got *bigger* when tightened**: mean **+6.67 / +6.15 / +6.73 pp** at the 15% / 21% / 30% coverage targets against **+4.93** at R7b's loose `energy ≤ 0.50`, positive in all four instrument-halves at every target — so the chop signal is a property of the feature, not of a permissive ceiling, which is what R7, R7b and R7c all left open. **The second switch cleared too**, and M29's fix is why: all three cells traded 107 / 159 / 223 round trips inside the 26–231 band where R7c's five fired at 250–385 (M32). **Then the machine lost.** It selected at **−0.3710** Sharpe on the half it was selected on, and on **SOL, spent, whole frame**: **−39.58% / −0.4800 / −90.22% at 3× costs** — beating R5's unmodified cell, losing to `donchian`, dying at 3×. DSR **0.1416** against a declared 0.95. Outcome row 4 as declared: **`energy` discriminates chop at matched selectivity, trades, and still does not earn — the problem is execution, not estimation** (M31). Caveat that must travel with the table: `donchian`'s SOL Sharpe of +0.4954 is an artifact of a book that went **negative** (net −104.80%), detected in code rather than prose. **R8 does not start**; the remaining unbuilt value is the live path. See [§9.9](#99-r7d-the-lift-at-matched-selectivity--the-holdout-spent-and-failed). |
| 2026-08-06 | **R7c stopped at its kill switch, and the SOL holdout was not spent — which turned out to matter, because the grid could not express the hypothesis it was testing.** Pre-registered at `abd2ebc` with the tradeability check placed *before* the holdout run. The selected cell traded **256** round trips against a declared band of **26–231**, and every constructible cell fired it (250, 256, 314, 328, 385), so steps 4–6 never ran and the code for them was never written. Controls first, all passing: the mode off reproduces R5's row, all four v1/v2 figures are bit-identical to a `main` worktree baseline, 925 tests. **The defect is in the pre-registration and it is mine.** The grid was declared in `energy` units against a gate declared in `strength` rank units: `strength ≥ 0.80` advances on **21.1%** of measurable training bars while the grid's *tightest* cell advances on **37.9%**, and a selectivity-matched threshold is **≈0.155, 56% below the declared floor** (M29). So a turnover band anchored on R5's 77 trades was near-unreachable by construction, and **the energy-first hypothesis is not falsified at matched selectivity — that was never tested**. The grid was *not* extended after the switch fired (M22/M23). Two things keep it from being a void run: the best cell scores **+0.1845 against R5's +1.3778** with four of five negative and **every cell between −48% and −86% at 3× costs**, and a coverage defect predicts more trading rather than a 42% drawdown. **SOL is still clean for the re-run** (M30). See [§9.8](#98-r7c-the-energy-first-lifecycle--stopped-at-the-kill-switch). |
| 2026-08-06 | **R7b: the diagnostic gate failed, so the P&L was never read and the SOL holdout is unspent — and the failure is an inversion rather than a refutation.** Pre-registered at `25b7509`, thresholds as numbers, with the diagnostic declared to run *before* any return and to kill the phase on its own. It did. Three controls first, all passing: `energy_ceiling = 1.0` reproduces R5's row (+15.4543% / +0.8956 / 4.6689% / 73); **all four published v1/v2 figures are bit-identical to `main`'s**, compared as floats against a baseline regenerated from a clean `main` worktree; both safety suites plus the exposure suites pass (875 tests, was 829). **The verdict: 0/4 on BTC, 0/4 on ETH**, at every ceiling, with 0/8 more at H=6 and H=90. But the composite beats `strength` alone at every ceiling in BTC's training half and in **both** of ETH's halves at 0.50 and 0.65, while beating `energy` alone **nowhere** — so **`energy` adds to `strength` and `strength` does not add to `energy`** (M27). Alone at H=30, `energy ≤ 0.50` lifts the trend rate **+3.51 / +5.10** (BTC) and **+4.49 / +6.61** (ETH), positive in all four instrument-halves; `strength ≥ 0.80` reads **−7.63 / +3.32** and **−1.26 / −2.52**, negative in three of four — **including the half R5 selected it on**. Not a declared threshold, so a lead and not a result. Embargo re-checked on the rate metric: 31 training rows cross the split on each frame, worth 0.57 / 0.48 pp against a 5 pp bar, verdict unchanged. Outcome row 3, with the caveat that row 4's *reading* is broader than the one gating actually tested. See [§9.7](#97-r7b-the-energy-gate--btcusdt-and-ethusdt-perp-4h). |
...
| 2026-08-10 | **R11: a short-horizon direction feature, measured and rejected.** R10i left the registry with no directional read that works below a ~2-week horizon — the shipped `direction` is +0.0464 at 4h against −0.0207 at 15m and −0.0196 at 1h on BTC/USDT spot (h=30), and shortening its spans at 4h degrades it monotonically on all three stored perps. `thrust` was pre-registered as the alternative shape: ranked range position times a ranked displacement-over-true-range term, no EWM anywhere — a 24-bar feature with a **47-bar warmup** against `direction`'s 96/1,920. It **failed two of its three pre-committed gates** and was deleted rather than kept unregistered, since the registry is what enrols the poison probe. **G1** (\|IC@6\| ≥ 0.03 on BTC *and* ETH perp 4h): **−0.0117 / −0.0192**, both under. **G2** (halves agree in sign): BTC h=6 splits **−0.0283 / +0.0090**. **G3** (\|corr\| with `direction` < 0.7): passed at +0.32 on both — a genuinely different statistic, just not an informative one. SOL was never looked at; the holdout was not reached. **The one consistent thing it showed is a lead, not a result:** at h=30 both halves agree in sign on both instruments and the sign is *negative* (BTC −0.0110, ETH −0.0050) — sitting high in your own recent range predicts a slightly lower forward return. Four instrument-halves agreeing is more than several registered features manage, and it is mean reversion rather than trend, but at \|IC\| ~0.01 against `direction`'s 0.04 it is far too small to carry anything alone. Whether it hides a larger conditional effect the way R4's `direction` did inside a `strength` tercile is a different question and a different pre-registration. Nothing about `direction`, either state machine, or `_EWM_WARMUP_MULTIPLE` moved. |
| 2026-08-10 | **R12: the EWM warmup multiple, 20x to 5x — 2.6x the usable chart for zero states changed.** `_EWM_WARMUP_MULTIPLE = 20` came from one measurement about **bit-exactness** (0/300 probed bars inexact at 20x, 41/60 still inexact at 18x) and had been carried as a design constant since. It is not an information requirement: a span-96 `ewm(adjust=False)` puts half its weight on the last **34 bars** and 99% on the last **222**, and the bar 1,920 back carries **8.9e-20** — four orders of magnitude below float64 epsilon, so it cannot influence the value at all. **5x rather than lower because 5x, 4x and 3x all declare the same 847-bar warmup**: below 5x the deepest feature stops being `direction` and becomes `strength`/`stability` at `96 + 479 = 575`, so going further only degrades the EMA inside a masked region. **G1 passed exactly** — 0 states differ over the 52,864 bars both settings can compute across BTC/ETH/SOL perp 4h and BTC spot 4h. Every trade-derived figure is unchanged (R5, on the gate's own configuration: 73 trades against 73, 6,048 tradeable bars against 6,048, net delta 2.1e-14, max-drawdown delta 1.3e-13); the **only** figures that move are Sharpes, because that is the one statistic computed over the whole equity curve and the curve is now 1,345 bars shorter — masked warmup the strategy could not trade was dragging it down. **v1 trained +0.896 -> +0.979, v1 default +0.746 -> +0.816, v2 default +0.913 -> +0.998**; v2's trained cell (+0.842) and the training-surface Sharpes were not re-run and still carry the old denominator, which STRATEGIES.md says out loud. **What was given up is real**: a cold start is no longer bit-exact, with a residual of 9.2e-06 relative on an adversarial synthetic frame and 5e-08 on real ones. It reaches no decision — 0 state differences over 164 cold-start probes on four instruments — so the guarantee moved from *provable* to *measured*, and the test that used to assert bit-exactness now asserts the bound and records where bit-exactness arrives. `ema_cross` keeps its own 20x. **A Sharpe on this program was never a property of a strategy alone** — it depended on how much warmup sat in front of it, which is the finding worth carrying forward. |
...
### 9.7 R7b the energy gate — BTC/USDT and ETH/USDT perp 4h

**Pre-registered before any figure existed**, at `25b7509`, one commit before the code. The
change, the three frames, the grid, the thresholds **as numbers** and the outcome table were
fixed there.

**The diagnostic gate failed, so the P&L was never read.** That is the pre-registration
working: the selection, the three evaluations and the DSR were not run, and `scripts/r7b/`
ships two steps rather than five. **SOL was never touched** — the holdout is still unspent.

**The three controls, all passing, before any R7b number.**

1. **The no-op is a no-op.** `energy_ceiling = 1.0` gives +15.4543% / +0.8956 / 4.6689% /
   73, reproducing R5's published row.
2. **Byte-identity against `main`**, captured from a clean worktree of `main` and compared
   as floats rather than to a tolerance — all four published v1/v2 rows bit-identical:
   `15.454329225912566 / 0.8955647910313967 / 4.668881364115885 / 73` and the other three.
   The constraint that has held since R2 still holds.
3. **Both safety suites pass** for the modified machine, plus the exposure suites: 102
   tests, and 875 in the full suite against 829 before.

**The change**, for the record even though it did not earn its place: one predicate,
`advancing &= (energy <= energy_ceiling)`, with `energy` added to `DEFAULT_FEATURES` and
`REQUIRED_COLUMNS`. `direction_floor` **stays** — R7 measured it inert in the entry gate but
it also governs the reversal test. `energy_ceiling` defaults to **1.0**, which is why
control 2 passes by construction rather than by inspection.

**The verdict — 0/4 on BTC, 0/4 on ETH.** Declared: the composite beats **both**
single-feature gates by ≥ 5 pp, in both halves, at H=30. Gap in pp, train / test:

| `energy_ceiling` | BTC vs `strength` | BTC vs `energy` | ETH vs `strength` | ETH vs `energy` |
|---|---|---|---|---|
| 0.50 | +5.58 / +4.26 | **−5.55** / +2.47 | +10.70 / +5.21 | +4.95 / **−3.92** |
| 0.65 | +6.30 / +1.41 | **−4.42** / +1.72 | +9.27 / +7.05 | +5.14 / **−1.88** |
| 0.80 | +5.40 / −0.13 | **−4.36** / +1.46 | +5.33 / +4.44 | +2.06 / **−2.04** |
| 1.00 (control) | +0.00 / +0.00 | −7.63 / +3.32 | +0.00 / +0.00 | −1.26 / −2.52 |

0/8 at H=6 and H=90 as declared non-verdict context.

**The failure is asymmetric, and the direction of it is the finding.** The composite beats
`strength` alone handsomely — every ceiling clears 5 pp in BTC's training half, and on ETH
0.50 and 0.65 clear in **both** halves. It never beats `energy` alone. **So `energy` adds to
`strength`; `strength` does not add to `energy`.** That is the mirror image of the
hypothesis, and it is the same shape R7 found for `direction_floor`, now found for
`enter_strength` — the axis R9 measured stable across all nine walk-forward folds.

**The single gates, H=30, trend-rate lift in pp against the base rate:**

| gate | coverage | BTC train / test | ETH train / test |
|---|---|---|---|
| **`strength ≥ 0.80`** — what the machine gates on | 21.7% | **−7.63 / +3.32** | **−1.26 / −2.52** |
| **`energy ≤ 0.50`** — what it does not read | 51.1% | **+3.51 / +5.10** | **+4.49 / +6.61** |

**`energy ≤ 0.50` is positive in all four instrument-halves; `strength ≥ 0.80` is negative
in three of four.** The gate the machine actually rests on selects bars *less* directionally
efficient than the base rate in BTC's own training half (−7.63 pp) and in both of ETH's.
**This was not a declared threshold and is therefore a lead, not a pass** — M23's discipline
cuts against R7b's own most interesting number as much as it cut against R9's.

**The embargo, re-checked** because R7b's verdict is a rate where R7's caveat 6 measured an
IC: 31 of BTC's 9,070 training rows and 31 of ETH's 8,602 have a forward window crossing the
split. Dropping them from both the tercile and the rates moves the largest gap by **0.57 pp**
(BTC) and **0.48 pp** (ETH) against a 5 pp bar. No cell clears either way.

**Purging and embargo proper: stated, not performed**, unchanged from R9's reasoning and
re-checked — the machine fits nothing, has no labels, and **no selection happened here at
all**.

**Outcome row 3**, as declared: *the diagnostic fails, the hypothesis is dead and the P&L is
not read.* Row 4's condition also holds, since R7's null stands untouched — but **row 4's
*reading* is broader than what was measured** and must not be quoted as if it were. R7b
tested **one** gating at four ceilings, not "any gating of the nine features".

**Caveats.**

1. **The measurable set was underdetermined, and it mattered.** The plan says the threshold
   is "reused verbatim from `scripts/r7`", but R7's `defined` set was `strength ∧ direction`.
   Scoring each gate on its *own* inputs would start 1,345 training bars earlier (`energy`
   warms in 503 bars against `direction`'s 1,920) and read the strength gate at 0.2642
   rather than R7's published 0.2513. Resolved to the machine's own `measurable` — all of
   `REQUIRED_COLUMNS` — which is the set the machine walks and which reproduces R7's row
   exactly. **The verdict is FAIL under either choice.**
2. **The plan does not say which instrument §1 is judged on.** BTC was fixed as primary and
   ETH as the replication before running, from the plan's own "selection happens here" and
   "confirmation" language. Both fail, so nothing turns on it.
3. **`state_machine_v2`'s byte-identity needed a slice the charter does not state.** §9.3's
   Sharpe is over the whole window including the flat warmup — that is what reproduces v1's
   +0.8956; the tradeable slice gives +0.9804 instead. Same-vs-same across the change is
   exact under either.
4. **The `main` baseline is regenerable, not committed** — `step0_control.py` with
   `R7B_BASELINE=1` on a `main` worktree. It was regenerated independently while verifying
   this section rather than taken on trust.
5. **`tests/test_state_machine.py` went 20 s → 52 s.** The full four-way cross of ceilings
   against the tail set was run once, passes with identical verdicts in 122 s, and the
   shipped parametrization crosses only where a ceiling can bite.

### 9.8 R7c the energy-first lifecycle — stopped at the kill switch

**Pre-registered before any figure existed**, at `abd2ebc`. **The kill switch fired, so the
SOL holdout was not spent** — steps 4–6 were never run and the code for them was never
written, on R7b's standard that a harness carrying unexecuted steps is indistinguishable
from one whose numbers were withheld. `scripts/r7c/` ships one step.

**This is M28's first real payoff, and it saved the holdout from a mis-specified
experiment** — see the defect below, which is in the pre-registration rather than in the
run.

**Controls, all passing.** Energy-first mode off reproduces R5's row (+15.4543% / +0.8956 /
4.6689% / 73); all four published v1/v2 rows are bit-identical to a baseline regenerated
from a clean `main` worktree; 102 safety and exposure tests pass, 925 in the full suite
against 875.

**The change**, recorded even though it did not earn its place: an energy-first mode moving
**both** sides of the lifecycle at once — `advancing` on `energy ≤ enter_energy`, `failing`
on `energy > exit_energy`, with the constructor enforcing `exit_energy > enter_energy` as
the mirror of `enter_strength > exit_strength`. `direction` keeps deciding side. Off by
default, which is why control 2 passes by construction.

**The surface, BTC training half, M21's scalar, 6,910 tradeable bars:**

| cell | net % | Sharpe | maxDD % | trades | net @3× |
|---|---|---|---|---|---|
| `enter≤0.35 exit>0.65` | −2.93 | −0.0150 | 22.38 | 250 | −54.05 |
| **`enter≤0.35 exit>0.80`** (selected) | **+4.88** | **+0.1845** | 19.58 | **256** | −48.11 |
| `enter≤0.50 exit>0.65` | −16.02 | −0.2427 | 31.54 | 314 | −75.11 |
| `enter≤0.50 exit>0.80` | −8.51 | −0.0805 | 26.41 | 328 | −71.35 |
| `enter≤0.65 exit>0.80` | −21.34 | −0.2973 | 42.34 | 385 | −85.86 |
| `(0.65, 0.65)` | *refused by the constructor* | | | | |
| **R5 trained**, reference | **+30.52** | **+1.3778** | 5.25 | **77** | **+20.39** |

**The kill switch: fired.** 256 round trips against the declared band of **26–231**, and
**every** constructible cell fires it (250, 256, 314, 328, 385), so the verdict does not
depend on which cell won. The band's anchor was re-measured rather than copied: R5's cell
trades exactly 77 on the identical bars.

**The pre-registration's grid was never selectivity-matched to the gate it replaces, and
that is a defect in the plan.** `energy` is a rolling percentile, so `energy ≤ x` covers
about *x* of bars. Measured on the 7,150 measurable training bars: `strength ≥ 0.80`
advances on **21.1%**, while the grid's *tightest* entry, `energy ≤ 0.35`, advances on
**37.9%**. A selectivity-matched threshold is **`enter_energy ≈ 0.155` — 56% below the
declared floor**. The grid therefore never contained a cell as selective as the one it was
testing against, and a turnover band anchored on R5's 77 trades was close to unreachable by
construction. **The grid was not extended after seeing the switch fire**, which would be
exactly the M22/M23 failure this program keeps recording.

**So what R7c licenses is narrower than its outcome row.** The declared row lands — *the
inversion does not produce a book, SOL is not spent, and M27 stands as a statement about
rates that no lifecycle has been able to trade* — but **the energy-first hypothesis is not
falsified at matched selectivity, because matched selectivity was never tested.**

Two things cut the other way and are why this is not simply a void run. The Sharpe column
is independent of the turnover band: the best cell scores **+0.1845 against R5's +1.3778**
in sample, four of five are negative, and **every cell is between −48% and −86% at 3×
costs**. And the coverage defect predicts *more* trading, not worse risk-adjusted return —
a 42% drawdown at `enter≤0.65` is not a turnover artifact.

**Purging and embargo: stated, not performed**, a fortiori — no out-of-sample evaluation
happened at all.

**Caveats.**

1. **The grid is described as "6 cells" while the constructor is required to refuse
   `exit_energy = enter_energy`.** `(0.65, 0.65)` is both, giving 5 live cells. Resolved
   toward the refusal, as the more specific commitment and the one the plan asks to be
   tested. Nothing turns on it.
2. **The reference Sharpe is +1.3778, not R9's published +1.3974.** Same book, different
   scoring window: R9 scored all 54 cells from the grid's deepest warmup (2,352 bars) for
   comparability, while every R7c cell warms 2,160. The **trade count the band is built on
   is 77 either way**, and no published figure moves.
3. **SOL's candles were read once during setup** to confirm 12,914 bars and zero funding
   gaps — already-published metadata, no returns computed, no backtest run.

### 9.9 R7d the lift at matched selectivity — the holdout, spent and failed

**Pre-registered before any figure existed**, at `94e2969`. **Both kill switches cleared,
SOL was spent, and it failed.** `scripts/r7d/` is the whole diff — **no `src/` change
was needed**, since R7c already shipped the energy-first mode and
`require_comparable_windows`.

**Controls, all passing.** Energy-first off reproduces R5's row (+15.4543% / +0.8956 /
4.6689% / 73); all four published v1/v2 rows bit-identical to a baseline regenerated on a
clean `main` worktree, compared by `repr`; 928 tests, the same collection count as `main`.

**The derivation, which is the phase's methodological point.** The grid was declared in
**coverage** and each frame derived its own `energy` value from the target (M29, M18):

| frame | enter target | value | realised | exit target | value | realised |
|---|---|---|---|---|---|---|
| BTC | 15% | 0.1083 | 15.05% | 30% | 0.2667 | 30.08% |
| BTC | 21% | 0.1667 | 21.02% | 42% | 0.4062 | 42.18% |
| BTC | 30% | 0.2667 | 30.08% | 60% | 0.5938 | 60.15% |
| ETH | 21% | 0.1708 | 21.21% | 42% | 0.3604 | 42.11% |
| SOL | 30% | 0.2479 | 30.23% | 60% | 0.5667 | 60.13% |

Same coverage, different values per instrument — which is the whole reason coverage rather
than the threshold is what gets declared.

**1 · The first kill switch: cleared, and the lift is *larger* when tightened.** Control
first: `energy ≤ 0.50` reproduces R7b's published lifts exactly. Trend-rate lift in pp at
H=30:

| enter target | BTC train | BTC test | ETH train | ETH test | mean | verdict |
|---|---|---|---|---|---|---|
| `energy ≤ 0.50` (R7b's loose ceiling) | +3.51 | +5.10 | +4.49 | +6.61 | +4.93 | — |
| **15%** | +6.56 | +5.91 | +3.46 | +10.76 | **+6.67** | **PASS** |
| **21%** | +4.70 | +5.74 | +4.23 | +9.92 | **+6.15** | **PASS** |
| **30%** | +5.04 | +7.75 | +5.70 | +8.43 | **+6.73** | **PASS** |

**3/3 clear, and every mean exceeds the loose threshold's +4.93.** The bar allowed the lift
to lose ~40% of its size to tightening; instead it **grew**. So `energy`'s chop
discrimination is a property of the feature, not an artifact of a permissive ceiling — the
question R7, R7b and R7c all left open. Context, not the verdict: all three clear at H=6,
none at H=90 (BTC training is negative there). Embargo re-checked: largest change 0.22 pp
against a 3.0 pp bar, no verdict moves.

**2 · Selection and 3 · the second kill switch: cleared — and M29's fix is what did it.**
BTC training half, 6,910 tradeable bars, M21's scalar:

| cell | net % | Sharpe | maxDD % | trades | net @3× |
|---|---|---|---|---|---|
| enter 15% / exit 30% | −20.37 | −0.9007 | 22.39 | 107 | −39.74 |
| enter 21% / exit 42% | −22.19 | −0.7602 | 25.40 | 159 | −50.87 |
| **enter 30% / exit 60%** (selected) | **−13.48** | **−0.3710** | 30.45 | **223** | −55.33 |
| **R5 trained**, reference | **+30.52** | **+1.3778** | 5.25 | **77** | **+20.39** |

**Every cell lands inside the 26–231 band (107 / 159 / 223)** where R7c's five all fired at
250–385. M29's diagnosis was right and its fix sufficient: declaring the grid in coverage
put the whole grid where a turnover band anchored on R5 could reach it. **But the selection
is the least-bad of three negative cells** — the machine trades, and loses, on the half it
was selected on.

**4 · The holdout, spent.** SOL/USDT perp 4h, 12,914 bars, whole frame:

| run | net % | Sharpe | maxDD % | round trips | net @3× |
|---|---|---|---|---|---|
| **energy-first (R7d)** | **−39.58** | **−0.4800** | 45.00 | 309 | **−90.22** |
| R5 trained, unmodified | −38.07 | −0.5168 | 46.40 | 156 | −60.17 |
| `donchian` 40/10 | −104.80 | *+0.4954* | 121.66 | 28 | −101.22 |

Against the declared threshold: beats R5 trained ✔, beats `donchian` **N/A**, survives 3×
costs ✘ → **FAIL**. The `donchian` comparison is not merely lost but **incomparable** — its
book crossed zero, so there is no risk-adjusted return to beat (caveat 1), and the harness
records it as such rather than entering an artifact into the verdict as a failed test. The
holdout fails on the cost-stress row, which is independent of it. The plan's SOL derivation
was ambiguous; both readings were run and agree (−39.47% / −0.4996 under the other).

**5 · Deflated Sharpe.** Three trials at 4h bar frequency: observed **−0.3710** against
`E[max SR]` **+0.2340**, z −1.0735, **DSR 0.1416** against the declared 0.95. Both
conditions fail — though a DSR on a search whose winner is *negative* is measuring how
easily noise beats a losing rule, which is not the question it was declared for.

**6 · Purging and embargo: stated, not performed**, reasoning re-checked against the code.

**Outcome row 4 lands, as declared: `energy` discriminates chop at matched selectivity,
trades, and still does not earn out of sample — so the program's problem is execution
rather than estimation.**

**Caveats.**

1. **`donchian`'s SOL Sharpe of +0.4954 is an artifact and must not be quoted.** Its net
   return is **−104.80%**, i.e. the book went **negative** — final equity −479.51 against
   10,000 initial, and 9,640 bars at or below zero — because sizing is non-compounding, the
   book was short, and the instrument multiplied. A percentage-return Sharpe across a sign
   change is not a risk-adjusted return. The holdout fails on its own 3×-cost row
   regardless, so the verdict does not rest on this comparator. The harness detects it in
   code (`ruined`) rather than only noting it in prose.
2. **The selected cell is negative in sample**, so §5's deflation prices a search among
   losing rules.
3. **SOL was evaluated twice, and the plan is why.** Its §"grid" says each frame derives
   its value on "the frame's own **training half**" while its §"frames" gives SOL **no
   split**, so for the holdout the two clauses do not pick out one derivation. The
   implementation resolved that *after* pre-registration — whole frame — and ran the 60%-cut
   reading as well. Both fail identically (−39.58% / −0.4800 against −39.47% / −0.4996), so
   no verdict turns on it, but **"the holdout was touched once" is not literally true of
   this phase** and the write-up above says "spent" rather than "spent once". Running both
   and reporting both is the honest response to an ambiguity I wrote; deleting the second
   measurement to make the sentence true would not be. **A future plan declaring an unsplit
   holdout states its derivation in one clause** — M33.
4. **M29's coverage figures were computed on a different denominator than the machine's** —
   see the correction attached to that row. Direction and magnitude of M29's finding are
   unchanged.

### 9.10 R10a the why-layer — census item (a), measured

**A design plan rather than a pre-registration** ([docs/plans/2026-08-06-r10a-why-layer.md](../plans/2026-08-06-r10a-why-layer.md),
`81d02a2`), because this phase produces a schema and a diff rather than a figure. Its
**declared readings were still binding**, and the diff landed on the first one — after
first landing on the second, for a reason that was the harness's own.

**The design decision that shaped it, and it inverts the obvious one.** The research path
already computes a `WhyLayer` per bar and is **right not to store it**: recompute is 19 ms
(Q6), the inputs are immutable candles, and a stored copy of a deterministic function of
stored data is a second record to drift. What is unrecomputable is what a **live** run saw
when it decided. So the event path persists, the research path keeps recomputing, and
R10's gate — *"research and live code paths produce identical output"* — becomes the
**diff between them**, per bar rather than per signal.

**The diff.** BTC/USDT perp 4h, right edge bounded by `funding_span` per the coverage rule,
**6,048 measured bars** — R5's own test-half length. Replay wrote 6,048 reason rows and 325
signals.

| | bars | differing | max abs delta |
|---|---|---|---|
| **`crowding`** | 6,048 | **6,048 (100%)** | **0.500000** |
| `direction` | 6,048 | **0** | 0 |
| `energy` | 6,048 | **0** | 0 |
| `stability` | 6,048 | **0** | 0 |
| `strength` | 6,048 | **0** | 0 |
...
State distribution, live against research: `RIDING` **848 / 515**, `EXHAUSTION` 224 / 338,
`COMPRESSION` 4,124 / 4,286. **The gap is behavioural, not cosmetic** — the live path spends
64% more bars in `RIDING` than the research path does over the same candles.

**Declared reading 1, and proved rather than eyeballed.** The plan's first row asks that
`crowding` differ, the other four agree, and the state differ *only where crowding moved a
transition*. The last clause is the one an eyeball cannot settle, so it was measured: **the
research features with `crowding` pinned to the live constant reproduce all 6,048 stored
states, 0 residual.** Verified independently of the harness. So **census item (a) is the
entire live/research gap for this strategy** — not the largest of several, the whole of it.

**It landed on reading 2 first, and the unenumerated gap was the harness's own storage.**
At `NUMERIC(38,18)` the other four features disagreed on 3–28 bars of 1,000 by exactly one
ULP. A `NUMERIC` scale counts decimal **places** where a float64 needs up to 17
**significant digits**, so a 1/480 rolling percentile (0.0020833333333333333, 19 places) and
`crowding`'s smallest reading (2.07e-05) are truncated. Unconstrained `NUMERIC` round-trips
all of them and a parametrized test pins it.

**That has a consequence outside this phase, and it is not fixed here.** `market_candles`'
price columns are `NUMERIC(38,18)`. CLAUDE.md's existing rule fixed the float8 → numeric
**cast**; the **scale** is a separate limit nobody had measured. The condition is on the
digits rather than the magnitude — a value truncates once its shortest round-trip decimal
runs past **18 fractional places** — so it needs a small price *and* full float64 precision
together. Measured: `0.05`, `0.0625` and `0.0052631578947368418` all round-trip, while
`1/480` and `2.0682314349096398e-05` do not. Recorded as M34, not repaired —
`market_candles` is read-only in this phase and repairing it is a migration with its own
controls.

**Controls.** All four published v1/v2 rows **bit-identical** to a baseline captured in a
clean `main` worktree, repr-for-repr; the no-op control reproduces R5's published row with
`crowding_measured=True`; **957 tests** (was 928), both safety suites and both exposure
suites included; `tests/test_migrations.py`'s two lock tests still pass, since no statement
touches `signals`.

**The schema.** `bar_reasons`, one row per `(run_id, strategy, instrument, timeframe, bar)`
— **no `side`**, unlike `uq_signals_identity`, because a bar has one state where a reversal
bar has two signals. Mirrors `signals` otherwise: `run_id` per invocation, both append-only
triggers (row-level **and** statement-level, since `TRUNCATE` bypasses the first), indexes
through `_guarded_index_migration`. Feature values are parallel `TEXT[]`/`NUMERIC[]` arrays
with a `cardinality()` CHECK — a column per feature cannot work because the set is a
strategy property, and JSONB would put the values back in the read-as-labels box
`runner._features` warns about. A NULL element is the warmup NaN, as SQL NULL rather than a
stored NaN, since Postgres accepts `NaN` in `NUMERIC` and nothing matches it afterwards.

**Caveats.**

1. **One strategy, one instrument.** `state_machine_v1` on BTC. `state_machine_v2` cannot
   run on the event path at all — census item (d) — so it is untested here and unfixed.
2. **Census items (b), (c), (d) and (f) remain untouched and unmeasured.** Only (a) and (e)
   are numbers now.
3. **The plan was underdetermined on one point.** "The runner extracts the reasons from the
   same `generate_signals` call, reading the last row" is not literally possible — a
   `SignalSet` carries no state or features. Resolved with the same buffer, the same feature
   functions and the same machine, via the `getattr` introspection the plan does mandate;
   the alternative was changing every strategy's return type. Cost measured rather than
   asserted: **6.8 s → 11.5 s over 1,000 bars (1.7×)**, with `record_reasons=False` as the
   opt-out.
4. **Reading 1 was reached only after fixing the storage defect in caveat above.** Both runs
   are in the record.

### 9.11 R10b the board — several instruments, one computation

**A design plan** ([docs/plans/2026-08-06-r10b-dashboard.md](../plans/2026-08-06-r10b-dashboard.md),
`fc811d1`) with six acceptance checks rather than thresholds. **All six pass**, and one of
its two offered implementations turned out not to exist.

`GET /api/board` streams newline-delimited JSON, one row per (dataset, strategy) as each
finishes: state, last feature values, latest fill, target, `as_of`, the newest stored bar, a
sparkline tail, and the full provenance block. The page opens on a grid of tiles with a
state pill, an inline-SVG sparkline, the fill, feature chips and per-row refresh; `open`
switches to the existing instrument view for that pair, which is otherwise unchanged.

**The load-bearing property, asserted rather than hoped for.** The board **slices**
`build_analysis` and never derives an answer a cheaper way, because a cheaper route would
be a **third** path free to disagree with the browser and the backtest — the same reason
the browser's markers are engine fills rather than raw signals. Check 2 asserts every
answered row's state, every last feature value, the latest fill and the whole provenance
block against `/api/analysis` re-asked over that row's **own** window. Verified
independently of the harness on all three answered perp rows.

**Measured, and it removes one of the plan's two options.** The plan allowed rows to arrive
"incrementally or in parallel". **Parallelism does not work here: four threads over the
three stored perp frames returned 1.10×**, because the work is pandas and vectorbt under
the GIL rather than waiting on anything. Streaming is therefore not a preference but the
only lever, and that constrains every later attempt to widen the board.

| check | result |
|---|---|
| 1 · row per dataset, refusals in-row | **pass** — 4 perp datasets, 4 rows; the 25-bar probe set reports its warmup refusal without blanking the other three |
| 2 · row ≡ `/api/analysis` | **pass** — verified independently; mutation-checked at `features[-2]`, `markers[-2]` and a dropped close |
| 3 · *(withdrawn)* every request recomputes | the board **was** cached on the last bar; review showed the stamp cannot see the in-place candle rewrite `/api/refresh` performs by design, so the cache is **removed** — see the correction below |
| 4 · *(withdrawn with check 3)* | invalidation is moot once nothing is held |
| 5 · a board **read** writes nothing | **pass** — 0 mutating statements on `GET /api/board`, `signals`/`bar_reasons`/`market_candles` counts unchanged. The page's refresh buttons call `POST /api/refresh`, which writes candles by design; the claim is about the read path |
| 6 · first paint under a second | **pass** — first row **67 ms**, all four **1,156 ms**, and that is now *every* request rather than a cold one |

**The tradeoff that is visible on purpose.** Bounding each frame's right edge at its last
settlement makes a perp row **up to one bar stale** — measured, ETH reads `as_of` 16:00
against a 20:00 stored bar, BTC 00:00 against 04:00. The tile shows both, in orange, rather
than hiding it. The alternative is an unbounded right edge, which turns any `fetch-crypto`
that advanced candles without funding into a **whole-board refusal** — the coverage guard
doing exactly what it should, at the cost of the entire view. A visible one-bar lag is the
cheaper failure (M37).

**Caveats.**

1. **The cache was removed under review, and checks 3 and 4 went with it.** It keyed on
   the newest stored candle plus the funding window — and `POST /api/refresh` upserts
   *overlapping* recent candles by design, so a corrective refresh that rewrote the last few
   bars without adding one left that stamp unmoved and the tile stale, while `/api/analysis`
   recomputed and disagreed. That is the one failure **M36** exists to prevent, introduced by
   the same phase that wrote M36, and CLAUDE.md's browser contract says plainly that the
   browser *recomputes from stored candles on every request*. A cache whose correctness
   depends on enumerating every writer is fine until one is missed. Every request recomputes
   now; check 6 is unaffected, because what makes the board answer quickly is the streaming,
   not the memo.
2. **The `open` control rounds `From` up to the next whole UTC day** when the funded window
   starts mid-day, because the instrument view's inputs are `type="date"`. On BTC that drops
   2 bars of a 2,192-bar warmup. The provenance strip states the actual frame either way,
   and check 2 compares against the row's own window, so nothing silently disagrees.
3. **Check 3's exact statement count includes `pandas.read_sql`'s catalog probe**, so it
   would go red for a non-reason if pandas changed that. The named-column assertion beside
   it is the durable one.
4. **Perps only.** Census item (f) — the Yahoo fetcher rewrites past bars on a dividend —
   makes an equity row's freshness a different problem, and it is still unmeasured.

---

### 9.12 R10c equities on the board — census item (f), and what a tile can honestly claim

**A design plan** ([docs/plans/2026-08-07-r10c-equities-on-the-board.md](../plans/2026-08-07-r10c-equities-on-the-board.md),
`b2e8486`) with six acceptance checks. **All six pass.** The board now covers the scope the
product vision opens with — crypto and stock/ETF — and the phase's finding is not the
widening but **what had to be said differently** once it widened.

**Census item (f) is now a number.** Against a fresh fetch of the stored SPY weekly series:

| | |
|---|---|
| overlapping bars, stored vs fresh | **333** |
| bars whose close changed | **333 (100%)** |
| median relative move | **0.257%** — largest **0.405%** |
| oldest bar that moved | **2020-01-01** |

So an equity history is **restated**, not appended: two figures computed from it on
different days are computed from different data for the same dates. **It reaches the
signals rarely and unevenly.** Run over both versions of the same 333 bars,
`trend_following_deepseek_v4` (21 entries) and `tsmom` (209) differ on **0** bars, because a
rescale cancels in a ratio — while **`donchian` differs on 3 bars, 39 entries against 38**,
because it compares a close against a channel's own high and low, and each bar's adjustment
factor reflects the dividends paid *after* it, so the rescale is not uniform and a marginal
break flips. The rewrite is small, real, and **strategy-dependent**, and nothing on a tile
told a reader which kind they were looking at.

**A perp tile and an equity tile therefore go stale for different reasons, and each states
only its own.** A perp frame is bounded on the right by its own stored funding, so what can
lag is its **right edge** (M37, unchanged). An equity frame runs to its newest bar and that
edge cannot lag; what can be stale is its **whole history**. `market_candles.updated_at` is
already maintained on every upsert, so the answer needed no schema change — one more
aggregate (`max(updated_at)`) on the group-by `stored_datasets` already runs. The second
half is a claim rather than an optimisation: **no `funding_span` query is issued for an
instrument that settles nothing**, because a window derived from an empty table is a
coverage guard invented for a market that has none (M38). `funding_window` is now perp-only
and *refuses* anything else; `board_window` is what dispatches. `DEFAULT_MARKET_TYPE` stays
`perp` — opening on a market whose tiles carry a restatement caveat is a choice a reader
makes rather than inherits.

| check | result |
|---|---|
| 1 · row per dataset, refusals in-row | **pass** — 10 equity datasets × 2 strategies = **20 rows**; the four 333-bar weeklies (`XLF`, `XLK`, `QQQ`, `SMH`) plus both SPY weekly sets report `state_machine_v1 declares 2192 warmup bars but the frame has 333` in their own rows while `donchian` answers the same datasets and the four hourly sets answer both |
| 2 · row ≡ `/api/analysis` | **pass** — all **14** answered rows re-asked over their own window: state, every last feature value, latest fill, last close and the whole provenance block minus `generated_at` — **0 mismatches**, and 4 carry a why-layer so the state comparison is exercised rather than comparing `None` to `None` (M36) |
| 3 · no `funding_span` for an equity | **pass, by statement** — equity board **0** statements touching `funding_rates` against **7** on the perp control in the same run; `board_window(equity)` returns the whole history, `funding_window(equity)` raises |
| 4 · each tile states only its own | **pass** — 8 of 10 equity sets flag `candles written` at 81 days and 2 do not at 4 days, so both states occur on real data; neither branch carries the other's line, and `'equity'` appears nowhere in the authored page script |
| 5 · a board **read** writes nothing | **pass** — 47 statements on the equity read path, **0** mutating; `signals` 23,623, `bar_reasons` 9,560, `market_candles` 146,550 unchanged |
| 6 · every R10b perp behaviour unchanged | **pass** — the perp board run on this tree and on `ddae16f` in a detached worktree is **4 rows, field for field identical** apart from `generated_at` and the new `last_written`; the one-bar lag is still there (BTC `as_of` 00:00 vs 04:00). First row **111 ms**, all four **1,179 ms**, against R10b's 67 / 1,156 |

**Caveats.**

1. **`state_machine_v1` on an equity is the M20 condition, and the warning missed it — now
   fixed (M39).** Measured: it answers on the four hourly equity sets with
   `crowding_measured=False`, `funding_attached=False` and `crowding` pinned at 0.5 on every
   bar — `with_funding_column` returns early for a non-perp, so `required=True` is never
   consulted and the machine runs on **three** of its four features. On a perp the same run
   would be *refused*. Both surfaces gated their warning on `market_type === 'perp'`, which
   was wrong **in both directions**: silent on that equity machine, and firing on `donchian`
   over a funded perp where nothing is wrong at all. The predicate is now
   `reads_crowding && !crowding_measured`, carried by a new `Provenance.reads_crowding`
   introspected from the strategy's declared features. Verified on stored candles — the two
   correct cells are unchanged and the two wrong ones flip:

   | | old gate | new predicate |
   |---|---|---|
   | `state_machine_v1` · SPY 1h equity | silent | **warns** |
   | `donchian` · BTC/USDT 4h perp | warns | **silent** |
   | `state_machine_v1` · BTC/USDT 4h perp (funded) | silent | silent |
   | `donchian` · SPY 1h equity | silent | silent |

   Seven mutants run, seven died, including both reversions to the market-type gate.
2. **`max(updated_at)` answers "the newest write in the set", not "every bar is this
   fresh".** `POST /api/refresh` re-fetches only the recent bars, so a refreshed equity tile
   can read *written just now* while most of its history carries an older write time. The
   label and its tooltip say exactly that rather than overclaiming; `min(updated_at)` would
   answer the other question and neither alone is the whole truth.
3. **The 31-day threshold is a judgement, not a measurement.** Nothing here detects a
   dividend — that needs a second stored copy to diff against, which R10a's rule refuses —
   so the flag says *"old enough that a distribution has plausibly landed since"*, never
   *"this was restated"*.
4. **The stale-flag logic is pinned by matching source text**, because there is no JS
   runtime in this repo. The reversed-comparison mutant does die, but textually. Every
   existing page test has this property.
...
### 9.14 R10e the event path runs every strategy and every contract

**A design plan** ([docs/plans/2026-08-07-r10e-event-path-completeness.md](../plans/2026-08-07-r10e-event-path-completeness.md),
`866d879`) whose acceptance checks were **numbers R10d had already measured**. All six pass.
Nothing here is crowding-aware — that is R10f — and no published figure moves.

**Census items (b) and (d) are closed; (c) closed as "no work needed".**

| check | result |
|---|---|
| 1 · runner exits ≡ engine exits under each canonical mode | **pass** — 18 (strategy, frame) pairs over BTC and ETH perp 4h, **0 differences**, 5,201 emitted signals compared. R10d's gaps, scaled to the 300-bar primed window: `trend_following_deepseek_v4` ~145 → **0**, `turnaround_v1`/`v2` ~20 → **0**, and the six already at 0 stay there |
| 2 · `state_machine_v2` runs on the event path | **pass** — `ExposureRunner` ships, and `tests/test_exposure_determinism.py` now **drives it** instead of its local stand-in, with all three comparisons intact (whole-history vs streaming, primed from mid-history, target-level equality) |
| 3 · a contract mismatch is refused at construction | **pass** — `TypeError` naming `ExposureRunner`, raised with **no bars fed**, which is what distinguishes the fix from the 2,192-bar bug |
| 4 · every published figure bit-identical | **pass** — 41 stat keys on `state_machine_v1` over the full funded BTC frame, **none moved** against a clean `main` worktree; 39 non-null, so the comparison is a real run |
| 5 · lookahead and determinism suites, both contracts | **pass** — 1,022 tests, up from 1,003 |
| 6 · no new persisted field for size | **pass** — `core/types.py` and `storage/` untouched by this branch |

**What the exit mode does, and what it deliberately does not.** `StrategyRunner` takes an
optional `ExitMode` and emits the exits it implies **by calling `engine._exit_signals` over
the buffer** rather than reimplementing it — M36 reaching a third path. Two consequences.
`setup_invalidation_stop` is **refused at construction**, because the engine applies it as an
intrabar `sl_stop` and no bar-close `Signal` encodes that; approximating it is how a replay
quietly stops matching the backtest it claims to reproduce. And `trend_structure` can begin
raising **mid-run**, since it refuses a frame whose `short_entries` are non-empty and a
growing buffer can satisfy that on any bar — the engine's behaviour, surfaced on the first
bar it is knowable.

**Without a mode the runner emits exactly what it emitted before**, so every stored replay
predating this phase still describes what its runner did.

**(c) closed without code, and the reasoning is the finding.** R10d measured `position_size`
recomputable from a cold buffer at **0 differing over 600 bars**, and the research browser
already renders it — `api/analysis.py` returns it and `browser/page.py` reads it. M35 says do
not persist what can be recomputed, and a `Signal.size` would hand **7 of 9** strategies a
field they can only ever leave `None`, which is precisely the argument `ExposureSignal`'s own
docstring makes about why `target_exposure` is not on `Signal`. The census's *"a live chart
cannot show how big"* was true of the signal stream and false of the product.

**Caveats.**

1. **Check 1 drives a 300-bar primed tail, not the whole frame.** Replay is quadratic — every
   post-warmup bar recomputes over the whole buffer — so a full-frame drive of nine
   strategies on two frames is hours. The window is primed so the buffer is full, which is
   R10d's CONTROL R shape, and `tests/test_runner_exit_modes.py` carries the per-bar parity
   on synthetic frames where the whole range is affordable.
2. **`MultiAssetRunner` takes one mode for the whole universe**, not one per instrument. A
   book whose instruments exit on different rules is a different design decision than that
   class makes; a caller that wants it builds the runners itself.
3. **A test of mine was wrong before the code was.** "Nothing is submitted through warmup"
   fails at `rebalance_threshold=0.0`, because a zero band submits on every bar — in the
   engine too. The claim is true only for a non-zero band, and it is now two tests: the band
   suppressing a target that sits at zero, and the runner suppressing warmup whatever the
   target says.

---

### 9.15 R10f funding on the event path — census items (a) and (e) closed

**A design plan** ([docs/plans/2026-08-07-r10f-funding-on-the-event-path.md](../plans/2026-08-07-r10f-funding-on-the-event-path.md),
`cc8cd68`) whose first check was a number R10a had already measured, so closing the gap had a
right answer rather than a direction. **All six pass. The census is closed.**

| | R10a measured | now |
|---|---|---|
| `crowding` differing | **6,048 / 6,048** | **0** |
| `direction`, `energy`, `stability`, `strength` | 0 | 0 |
| **state differing** | **488 (8.1%)** | **0** |

Measured with R10a's own harness over 1,308 compared bars, not a new one.

**The load-bearing decision is that the column's *presence* is the claim.**
`state_machine_core.build_feature_frame` decides whether crowding is real with
`FUNDING_COLUMN in df.columns`, so `BarBuffer` materializes the column **only when a bar
actually carried one**. An always-present NaN column would report `crowding_measured=True` on
a spot frame and feed the feature garbage — replacing a fallback that is *correct* off-perp
with a silent wrong answer, which is worse than the gap it closed. `Bar.funding_rate` is
`Decimal | None` beside `quote_volume` and `trades`, and `None` is what the buffer reads.

**A stream cannot change its mind.** Bars that stop settling raise rather than silently
narrowing the frame: dropping the column mid-run would run a *different strategy* from the one
the earlier bars ran, which is M20 in a feed rather than in a flag.

**Coverage is required only when the strategy reads funding**, the same question `sweep` asks
and asked the same way (`"crowding" in getattr(strategy, "features", ())`). Otherwise BTC's
permanent 40 h leading gap would make its own range unreplayable for `donchian`, which reads
no funding-derived feature. With `required=True` an uncovered perp **refuses with the
engine's own guard**, naming the fetch command; with `required=False` it gets no column at
all rather than a partial one, so a funding-reading strategy falls back to neutral and records
it — the pre-existing behaviour rather than a silently gross number.

| check | result |
|---|---|
| 1 · replay ≡ backtest on a funded perp | **pass** — crowding 6,048 → **0**, state 488 → **0** |
| 2 · (e) closed, non-vacuously | **pass** — the determinism suite runs on funded frames, and `test_the_comparison_can_see_a_crowding_difference` strips funding from the streamed side and asserts the signals move. It does |
| 3 · a market that settles nothing keeps its fallback | **pass** — no column, `carries_funding` False, and the mixed-stream refusal beside it |
| 4 · an uncovered perp replay refuses | **pass** — the engine's `FundingUnavailable`, same message, names `fetch-funding` |
| 5 · every published figure bit-identical | **pass** — three strategies × 41 stat keys against a clean `main` worktree, **none moved** (39/39/37 non-null) |
| 6 · full suite | **pass** — 1,029 tests, up from 1,022 |

**What R10a's controls now say, and it is worth reading carefully.** Two of them report
**FAIL**, and that is the harness working rather than the fix failing. CONTROL A asserts *"the
live path really ran crowding at the neutral constant"* — false now, by design, and the stored
values it prints are real rates rather than 0.5. CONTROL C asserts *"crowding is the only
cause of the state differences"* by pinning research crowding to the neutral constant, which
now moves 124 of 1,308 states because the live path is no longer neutral. The harness's
declared reading 3 — *"nothing differs → suspect the harness"* — fires, and the answer is that
the harness's premise is false by design. CONTROL B still passes and is what keeps the zero
honest: 1,308 of 1,308 bars away from 0.5 on the research side, against real stored rates on
the live side, agreeing to a max delta of 0.

**Caveats.**

1. **R10a's harness now describes a world that no longer exists**, and is left that way
   deliberately — its controls are the record of what the gap *was*, and rewriting them to
   pass would erase the measurement this phase is judged against.
2. **The determinism suite got slower**: 30 tests over funded frames, ~63 s against ~40 s.
   The frames are what make it able to fail.
3. **`ReplayFeed.from_database` defaults to `required=False`.** Correct-by-default would have
   been `True`, and it is not, because a feed does not know which strategy will read it. The
   `replay` CLI asks the strategy and passes the answer; a caller that builds a feed by hand
   and forgets will replay an uncovered perp crowding-neutral, exactly as before this phase.
   That is the one place the old behaviour survives, and it survives on purpose.

---

### 9.16 R10g the book — signals into positions, gated on `trades.csv`

**A design plan** ([docs/plans/2026-08-08-r10g-the-book.md](../plans/2026-08-08-r10g-the-book.md),
`5734e8f`) whose gate is an artifact the repo already trusts. **All six checks pass.**

The census closed the *signal* gap; this closes the one `StrategyRunner._extract` has named
since Phase 1a — signals were recorded, not driving positions, and there was **no book in
`src/`**. Built before the live feed deliberately: **the book has an oracle and the feed does
not.**

| check | result |
|---|---|
| 1 · reproduces `trades.csv` trade for trade | **pass** — `donchian` **19/19** and `turnaround_v1` **355/355** closed trades on stored BTC perp 4h: entry bar, exit bar, direction, size and *both* fill prices, **0 mismatches** |
| 2 · a reversal is actually exercised | **pass** — 2 reversals in `donchian`'s range and 322 in `turnaround_v1`'s, asserted rather than hoped for |
| 3 · the continuous book matches `from_orders` | **pass** — `state_machine_v2`, **12/12** orders: same bar, same size, same side |
| 4 · non-compounding holds | **pass** — a book that has doubled its equity sizes the next entry identically |
| 5 · published figures bit-identical | **pass** — 41 stat keys × 2 strategies against a clean `main` worktree, none moved |
| 6 · full suite | **pass** — 1,043 tests, up from 1,031 |

**The execution semantics are derived, not chosen.** Measured against the installed vectorbt
with a controlled 12-bar signal set: size off the **raw** close, buy fills at
`close × (1 + slippage)` and sells at `(1 − slippage)`, fee on the filled notional, repeated
same-side entries ignored, and **a short entry while long closes the long *and* opens the
short — two trades on one bar, both at the sell price.** That last rule is the one a book
gets wrong by guessing, which is why check 2 requires a real reversal rather than permitting one.

**What the oracle found that reasoning did not.** The first run matched 19 of 19 trades on
bar, direction and both prices — and differed on **one size**. The engine had filled 9,158.96
of a requested 9,500 notional. Sizing is non-compounding in the **request** and **clipped by
cash in the fill**: measured exactly, `filled = min(requested, balance / (price × (1 + fee)))`,
with the balance moved by `−(qty × price + fee)` on a buy and `+(qty × price − fee)` on a
sell. So the book carries a cash balance, for that one purpose and no other. No amount of
reading the sizing code would have produced this; it is the same clipping the `max_weight`
warning describes, reached from the other side.

**Caveats.**

1. **The book holds an open position as a position, not a trade.** The engine reports it in
   `trades.csv` with `Status: Open` — 80 rows against 79 closed for `tsmom` — so the
   comparison is against closed trades only. Counting an open position would compare
   something the engine does not.
2. **No funding accrual in the book.** The engine charges carry as a separate ledger against
   held notional, and duplicating it here would be a second answer to a question
...
5. **The exposure clip's divisor was copied from the boolean path and is now verified on its
   own.** `balance / (price × (1 + fee))` was measured against `from_signals`; `ExposureBook`
   runs against `from_orders`, whose cash bound is different machinery, and the 938/938
   comparison could not check it because the bound never binds on stored data. A synthetic
   frame that forces it — max long held through an 85% crash, then max long asked for again —
   fills **125.769696 against a requested 633.33**, and the book matches the engine exactly.
...
- **Live bars against the record**: run 2's 3 closed bars, **0 differing** on
  `open`, `high`, `low`, `close`, `volume` **and `funding_rate`**.
- **Per-bar reasons**: **0 differing on the state and on every feature** —
  `crowding`, `direction`, `energy`, `stability`, `strength` — over 5 bars in run
  1 and 3 in run 2. This is R10a's diff pointed at a live run instead of a
  replayed one, and R10a's own reading was `crowding` differing on 6,048 of 6,048.

  **The feature set is read from the rows, not declared.** Under review the first
  version of this script was found to carry a hardcoded tuple naming
  `persistence` — a registered `StateFeature`, but not one this strategy reads —
  and omitting `stability`, which it does. A name absent from both sides compares
  equal to itself, so the wrong one was skipped in silence and the right one was
  never compared: **the run reported "every feature" having checked four of
  five**. The result above is the re-run after the fix and is unchanged, but the
  claim it supports was not the claim being made. Same shape as M44 and as
  reading 4 — a comparison that cannot fail, dressed as one that passed.
...
`0e53a9d`). `MarketDataFeed` had exactly one implementation from Phase 1a until now, and it
read from Postgres. **All six checks pass.**

**The phase's problem is that a live feed cannot be diffed against anything as it runs** —
which is why R10g built the book first, against a frozen `trades.csv`. It has a *delayed*
oracle, and that is the gate: a live bar and a stored bar for the same interval are **the
same fact recorded twice**, so a live window must equal a replay of that window, at bar
level and at signal level. A partial candle taken as final, a duplicate after a reconnect,
a timestamp off by a millisecond — each shows up as a difference against the record the
venue itself will serve tomorrow.

**It polls rather than opening a socket, and that is a decision rather than a shortcut.**
The protocol does not care where a `BarEvent` came from; the venue REST client already
exists and is tested; and at 4h and 1w the difference is not observable — the same argument
`browse` makes for its own refresh. A socket adds a dependency, a reconnect state machine
and a framing layer, all untestable without a network, to optimise a bar size nothing here
trades. It can land behind the same protocol later without touching the runner, the book or
any strategy.

| check | result |
|---|---|
| 1 · a live window equals a replay of it | **pass** — bars *and* signals identical, driven through the same `StrategyRunner` |
| 2 · the protocol's two unenforceable guarantees | **pass** — ascending, and no repeated `(instrument, timeframe, ts_open_ms, is_closed)`; a poll re-reads its window, so both are *maintained* here rather than inherited from an exhausted iterator |
| 3 · a forming bar is superseded, never duplicated | **pass** — one bar survives per interval, carrying the *closed* values, and `replaced_duplicates` counts it |
| 4 · a cold start reaches the replay's state | **pass** — `backfill()` primes a runner and lands byte-identical to the same history through `prime(DataFrame)` |
| 5 · no network in the suite | **pass** — `fetch` and `sleep` are injected; every check runs offline |
| 6 · published figures bit-identical | **pass** — 41 stat keys against a clean `main` worktree, none moved; 1,056 tests |

**Two §12 follow-ups stopped being cosmetic and are closed.** `backfill()` yields `Bar`
while `prime()` took a DataFrame, so they did not compose and a live process could not warm
itself from history — fatal rather than awkward, since `state_machine_v1` emits nothing for
its first **2,192 bars**, a year at 4h. Both runners now have `prime_bars(bars)` with
`prime(frame)` as the stored-candles adapter over it. And `Subscription.include_forming`,
declared in Phase 1a and read nowhere since, finally decides something.

**The finding is about a test rather than the code, and it is the phase's most useful
result.** The supersede check passed while a **first-wins mutant in `BarBuffer.append`
survived** — because the scripted venue returned the *same row* for an interval's forming
and closed copies, so the two were identical and "superseded" was untestable. A real
in-progress bar has a different close from the one it settles at. Perturbing the forming row
kills the mutant. A fixture that is too clean is the same failure as a test that asserts
nothing, and it is harder to see: the assertion was there, the values it compared were not
distinguishable.

**Caveats.**

1. **The oracle is delayed, not immediate**, and that is inherent. A live bar can only be
   checked once storage holds the same interval, so a defect is caught on the next run
   rather than the one that produced it.
2. **The venue fetch itself is untested here.** `_fetch_recent` wraps the client the
   fetchers already use; the injected-fetch design is what makes everything above offline,
   and it means the network call has the coverage that client already had, not more.
...
| M27 | **`energy` adds to `strength`; `strength` does not add to `energy`. The machine gates on the weaker of the two axes** | R7b ([§9.7](#97-r7b-the-energy-gate--btcusdt-and-ethusdt-perp-4h)), against a bar declared before the data was seen, with the P&L unread because the diagnostic is what settles a claim about an *estimator*. The composite clears 5 pp over `strength` alone at every ceiling in BTC's training half and in **both** of ETH's halves at 0.50 and 0.65 — and clears it over `energy` alone **nowhere**. Alone at H=30, `energy ≤ 0.50` lifts the trend rate **+3.51 / +5.10** on BTC and **+4.49 / +6.61** on ETH — positive in all four instrument-halves — while `strength ≥ 0.80`, the gate the machine actually rests on and the one axis R9 found stable across all nine walk-forward folds, reads **−7.63 / +3.32** and **−1.26 / −2.52**: negative in three of four, and negative in the very half R5 selected it on. So the phase's hypothesis is **inverted rather than merely refuted** — the second axis is not a filter to bolt onto the first, it is the stronger one. Two limits on how far this goes. It is **not a declared threshold**, so it is a lead and not a result (M23 cuts against R7b's own best number). And **no P&L was run**, so nothing here says an energy-first machine would trade profitably — R7b measured which axis carries chop information, not which earns. What it does settle is that R8 has no business estimating a state vector whose entry gate is built on the axis that measures negative. | 2026-08-06 |
| M28 | A pre-registered **kill switch** is worth more than a pre-registered threshold, and R7b is the first phase to fire one | The plan put the diagnostic before the P&L and said the returns would not be read if it failed. It failed, and they were not read — the selection, the three evaluations and the deflated Sharpe never ran, and **the SOL holdout was never spent**. That is the direct application of R7's own lesson: a P&L number scores the estimator and the policy as one quantity, so a hypothesis about an *estimator* must be settled before a return is visible or the return will settle it. The cost was one wasted instrument fetch and the benefit is that SOL is still a clean holdout for whatever replaces this, which the next phase gets to declare rather than inherit spent. Every phase after this one states which of its measurements can kill it, and in what order they run. | 2026-08-06 |

| M29 | **A replacement threshold must be selectivity-matched to the one it replaces, declared in coverage rather than in feature units — or the comparison measures coverage and not the feature** | R7c ([§9.8](#98-r7c-the-energy-first-lifecycle--stopped-at-the-kill-switch)) declared its grid in `energy` units — {0.35, 0.50, 0.65} — against a gate it was replacing that is declared in `strength` rank units, 0.80. Measured afterwards on the training half: `strength ≥ 0.80` advances on **21.1%** of measurable bars while the grid's tightest cell advances on **37.9%**, and a matched threshold would be **`enter_energy ≈ 0.155`, 56% below the declared floor**. So the phase's turnover kill switch, anchored on R5's 77 round trips, was close to unreachable by construction, and every cell fired it. Both features here are 0..1 percentiles over a 480-bar window, which is exactly what made the mismatch invisible: numerically comparable units are not comparable *selectivities*, since one gate is a floor and the other a ceiling on differently-shaped distributions. **Review found a second door into the same failure**: those two windows are only equal by coincidence of defaults — `rank_window` is a live field on both adapters while `Energy.percentile_window` is not — so at `rank_window` 240 / 480 / 960 the strength gate admits 23.8% / 21.6% / 20.9% of bars while the energy gate stays pinned at 37.1%, and the *relative* selectivity of the two moves with a parameter only one of them reads. `require_comparable_windows` now refuses that combination outright, since M29's whole point is that comparability is measured rather than assumed from units. The rule this sets: **a grid replacing an existing gate is declared by the coverage it should have, and the feature values are derived from that on the training half before the plan is committed.** The grid was **not** extended after the switch fired — that is the M22/M23 failure — so the hypothesis stands untested at matched selectivity and needs a fresh pre-registration. *(— two corrections from R7d, neither changing the direction or magnitude of this row. **The coverages above use a per-column denominator, not the machine's**: on BTC's training half `strength ≥ 0.80` and `energy ≤ 0.35` read 21.07% / 37.92% dropping each column's own NaNs, against **21.76% / 37.22%** under the joint `measurable` predicate the machine actually walks — the ~16 pp gap this row is about survives either way. And **the fix was sufficient**: R7d re-declared the same grid in coverage and all three cells landed inside the turnover band, 107/159/223 against R7c's 250–385, so this was the binding defect and not merely a contributing one — see M32.)* | 2026-08-06 |
| M30 | The **holdout survived**, and that is what the kill switch was for | R7c would have spent SOL on a grid that could not express the hypothesis it was testing (M29). Because the tradeability check sat **before** the holdout run rather than after it, the phase stopped at step 3 and SOL is still clean — so the re-run with a matched grid gets a genuine out-of-sample instrument instead of inheriting a spent one. The ordering was declared in advance for exactly this reason, and it is the first time in this program that a pre-registration prevented a cost rather than merely recorded one. Every phase from here states which measurement can kill it, in what order the measurements run, and **what the kill switch protects**. | 2026-08-06 |

| M31 | **The estimator works and the machine cannot trade it. Four phases have now separated measuring state from earning from it, and that ends the state-machine line** | R7d ([§9.9](#99-r7d-the-lift-at-matched-selectivity--the-holdout-spent-and-failed)) is the phase that could have rescued this and did the opposite by clearing its own diagnostics. `energy`'s chop lift **grew** when tightened to R5's selectivity — mean **+6.67 / +6.15 / +6.73 pp** across the three coverage targets against **+4.93** at R7b's loose ceiling, positive in all four instrument-halves at every target — so the signal is a property of the feature and not of a permissive threshold, which is the one thing R7, R7b and R7c all left open. The machine built on it then traded **223** round trips (inside the band, unlike R7c) to a **−0.3710** Sharpe on the very half it was selected on, and lost the holdout: SOL **−39.58% / −0.4800 / −90.22% at 3× costs**. So the sequence is complete and consistent — R7 found the states carry no chop information, R7b found the axis that does and that the machine ignores it, R7c failed to test it properly, and R7d tested it properly and found the discrimination is real while the book is not. **The problem is execution, not estimation.** That is a different research programme from the one §2 describes, and §2's remaining unbuilt value is the live path. R8 does not start: a better estimator is an answer to a question this program has now measured four times and found is not the one it is failing. | 2026-08-06 |
| M32 | M29's fix was **sufficient**, not merely directionally right — a grid declared in coverage put every cell inside a band none could reach when declared in units | R7c's five cells traded **250–385** round trips against a 26–231 band anchored on R5's 77, and every one fired the kill switch. R7d re-declared the *same hypothesis* with the grid stated as enter coverage {15%, 21%, 30%} and the `energy` value derived per frame from the target: **107 / 159 / 223**, all inside, and the phase reached its holdout. So the coverage mismatch was the **binding** defect rather than one of several, and M29's rule is load-bearing rather than a caution. It also carried across instruments as intended (M18): the value hitting 30% coverage is 0.2667 on BTC and 0.2479 on SOL, and declaring the *value* would have shipped BTC's volatility distribution to a different market. Every future grid replacing an existing gate is declared this way. | 2026-08-06 |

| M33 | **A holdout declared without a split must state its derivation in one clause, or "touched once" stops being true of it** | R7d's plan said each frame derives its `energy` value on "the frame's own **training half**" and, separately, that SOL is evaluated **whole frame, no split**. For BTC and ETH those are consistent; for the holdout they name no single rule, so the implementation resolved it after pre-registration and ran **both** readings — whole-frame and a 60% cut. They agree (−39.58% / −0.4800 against −39.47% / −0.4996) and no verdict moves, but the phase evaluated its holdout **twice**, which is exactly the property the holdout existed to have. Reported rather than repaired: deleting the second run would make the sentence true by discarding a measurement, which is the inverse of what these pre-registrations are for. Two rules follow. **A no-split frame states its derivation explicitly**, not by inheriting a clause written for split frames. And **an ambiguity discovered mid-phase is resolved by running both and saying so**, never by picking the reading that flatters the result — the cost of the honest version is a weakened claim, and that is the cheaper of the two. | 2026-08-06 |

| M34 | **A `NUMERIC` scale counts decimal places where a float64 needs significant digits, so `NUMERIC(38,18)` silently truncates a value needing more than 18 fractional places** | Found by R10a's diff landing on its own reading 2 ([§9.10](#910-r10a-the-why-layer--census-item-a-measured)): four features that should have agreed bit-for-bit disagreed on 3–28 bars of 1,000 by exactly one ULP, because a 1/480 rolling percentile is 0.0020833333333333333 — **19 decimal places** — and 18 is all the scale keeps. Unconstrained `NUMERIC` round-trips it; `bar_reasons` uses that and a parametrized test pins it. **The consequence reaches further than this phase and is deliberately not repaired here.** `market_candles`' price columns are `NUMERIC(38,18)`. CLAUDE.md's existing precision rule fixed the float8 → numeric **cast** — `col::text::numeric`, and `Decimal(str(float(x)))` on every write — and that rule is still right; the **scale** is a second, independent limit that no test covers. The condition is on the digits rather than the magnitude: a value truncates once its shortest round-trip decimal runs past **18 fractional places**, so it takes a small price *and* full float64 precision together. Measured, `0.05`, `0.0625` and `0.0052631578947368418` all round-trip while `1/480` and `2.0682314349096398e-05` do not. The repo has never stored one, which is why it has never shown. Repairing it is a migration with its own controls, on a table this phase treats as read-only. | 2026-08-06 |
| M35 | **A record that can be recomputed should be, and only the unrecomputable is persisted — so R10's gate is a diff, not a second copy** | R10a's plan started from the census's "the `signals` table records configuration, not reasons" and found the obvious remedy half wrong. The research path already computes a `WhyLayer` per bar and `api/analysis.py`'s docstring already argued against storing it: recompute is 19 ms, the inputs are immutable candles, and a stored copy of a deterministic function of stored data is a second research record to fall out of step with the first. What **is** unrecomputable is what a live run saw at the moment it decided — funding that had not settled, a feed gap, a late bar. So the event path persists, the research path recomputes, and *"research and live code paths produce identical output"* becomes a **per-bar diff between them** rather than a per-signal one. Measured immediately: `crowding` differs on 6,048/6,048 bars, the other four on none, the state on 488, and pinning crowding to the live constant reproduces every stored state with **0 residual** — so census item (a) is the *entire* gap for this strategy rather than the largest of several. The census's six prose items are now two numbers and four still-unmeasured claims, which is what the next phase gets to scope from. | 2026-08-06 |

| M36 | **A new view slices the one computation; it never derives the same answer a cheaper way** | The charter already said the browser is *not free to disagree* with a backtest, which is why its markers are engine fills rather than raw `SignalSet` rows. R10b ([§9.11](#911-r10b-the-board--several-instruments-one-computation)) is the first surface to test that as a *general* rule rather than a fact about one page: a board tile needs only a state label and a sparkline, and deriving those directly would have been far cheaper than the ~300–500 ms full-history `build_analysis` it calls instead. It calls it anyway, because a cheaper route is a **third** answer free to drift from the other two, and nothing would catch the drift — the disagreement would appear as a tile quietly contradicting the chart it links to. Check 2 makes it enforceable rather than aspirational: every answered row's state, feature values, latest fill and provenance block are asserted equal to `/api/analysis` over that row's own window. **Any future surface — an alert, a mobile view, an export — slices the same call or it is not trusted.** The cost is bounded by **streaming**, not by computing less and not by caching: this phase first memoised each row against the newest stored candle, and review pointed out that `POST /api/refresh` rewrites overlapping recent candles by design, so the stamp could sit still while the answer moved — M36's own failure, committed by M36's own phase. The cache is gone; every request recomputes, as CLAUDE.md's browser contract already required. | 2026-08-06 |
...
| M41 | **On the continuous contract a `side` names the direction of the level, not an event — so a flip is one row, not two** | R6 settled that a level rides *beside* `side` rather than replacing it: `side` stays a CHECK on four events, a level is not an event, and a boolean strategy keeps writing the row it always wrote. R10e ([§9.14](#914-r10e-the-event-path-runs-every-strategy-and-every-contract)) had to say what that `side` then *means* when the runner emits one. It is the direction of the level submitted — positive `ENTER_LONG`, negative `ENTER_SHORT`, zero the exit of whatever was held, which is the one value that needs the previous one to be readable. **The consequence a reader has to carry: a flip from +0.5 to −0.5 is a single row.** The boolean path emits two for a reversal because `exit_long` and `enter_short` are separate booleans and neither says what replaced the position; here `target=-0.5` already says the book is short half, and a second row would be describing the same decision twice. The band is what decides *when* a row exists at all, and it is the engine's rule rather than a new one — a target reaches the book once it has moved `rebalance_threshold` from the last target **submitted**, never from the previous bar's and never from the realized position, so a streaming runner holds one float where the vectorized path needs a loop. | 2026-08-07 |
| M42 | **A column's *presence* is a claim, so a buffer materialises it only when the data really carries it** | R10f ([§9.15](#915-r10f-funding-on-the-event-path--census-items-a-and-e-closed)) had to put `funding_rate` on `Bar` and into `BarBuffer.frame()`, and the obvious implementation -- always emit the column, `NaN` where absent -- would have been worse than the gap it closed. `state_machine_core.build_feature_frame` decides whether crowding is real with `FUNDING_COLUMN in df.columns`, so an always-present column reports `crowding_measured=True` on a **spot** frame and feeds the feature garbage: a fallback that is *correct* off-perp, replaced by a silent wrong answer. So presence means measured, everywhere, and `Bar.funding_rate` is `None` rather than `NaN` for a market that settles nothing. **Two corollaries.** A stream cannot change its mind: bars that stop settling **raise** rather than silently narrowing the frame, because dropping the column mid-run runs a different strategy from the one the earlier bars ran -- M20 in a feed rather than in a flag. And **coverage is required only when the strategy reads funding**, the same question `sweep` asks, because a feed does not know its consumer and refusing every imperfectly-covered perp would make BTC's permanent 40 h leading gap unreplayable for `donchian`, which reads nothing funding-derived. **The general rule: when a value can be genuinely absent, encode absent as absent -- never as a neutral number in a column that exists.** It is the same rule the feature layer already follows by masking warmup to `NaN` rather than 0.0. | 2026-08-07 |
| M43 | **Non-compounding sizing is a property of the *request*; the *fill* is bounded by cash, and the two are not the same rule** | The repo's standing rule is that an entry is sized from *initial* cash so a winning account does not compound. R10g ([§9.16](#916-r10g-the-book--signals-into-positions-gated-on-tradescsv)) found the other half by building a book and diffing it against `trades.csv`: 19 of 19 trades agreed on bar, direction and both prices, and **one differed on size** — the engine had filled **9,158.96 of a requested 9,500** notional. A *losing* account cannot pay for the size the non-compounding rule asks for, and `from_signals` fills what it can afford rather than refusing. Measured exactly: `filled = min(requested, balance / (price × (1 + fee)))`, the balance moved by `−(qty × price + fee)` on a buy and `+(qty × price − fee)` on a sell. **So the rule is asymmetric** — a doubled account still asks for the original size, a halved one gets less than it asks for — and any second implementation of sizing that models only the first half diverges after the first drawdown, permanently and in one direction. This is the same clipping the `max_weight` warning describes for the sizing layer, arrived at from the opposite end. **The general rule: when a second path must agree with a first, diff it against the first's own frozen output rather than against a reading of its source.** Reasoning about the sizing code would not have produced this; one trade in nineteen did. | 2026-08-08 |
...
| A bar the venue revises keeps its first stored reason | **Opened by R10h**: `bar_reasons` is first-wins within a run by design — *"a stored record of what a run saw that a later pass can overwrite is not a record"* — while `StrategyRunner._reasons` replaces in place, so after a revision the two disagree and a later replay of the corrected candle agrees with neither. The paper summary counts revised bars so it is visible; deciding whether a correction is a new observation or an amendment is a storage question, not a command's |
...
| The event path carries no `position_size` onto a `Signal` | **Opened by R10h**: `StrategyRunner` emits `strength=None`, so a paper book fills every entry at scale 1.0 and a strategy that sizes per bar trades differently live than in its backtest — the one property R10g's book was built to have. Declared at startup rather than silently different |

...
**Status:** R0–R5 complete; R6 (continuous-exposure contract) is next
**Owner:** Jingbo
**Created:** 2026-08-03
...
| R5 | Rule-based state machine | Beats R0 baseline out-of-sample, with hysteresis + dwell time + cooldown | R4 | **Done — gate passes.** OOS Sharpe +0.938 against the baseline's +0.072, max drawdown 8.24% against 43.86%; the untuned default also passes at +0.738. 54 configurations on the training half. **Wins on risk, not return** (+23.3% against buy-and-hold's +85.8%), loses to the test-half oracle donchian cell, and dies at 3× costs. The result is the follow band, not the mid-tercile fade. See the R5 row in §9 and [§9.2](#92-r5-split-sample-gate--btcusdt-perp-4h). |
| R6 | Continuous-exposure contract | Second strategy contract; vectorbt replaced for this path | R5 | **NEXT** |
...
| H3 | The state model mainly improves **sizing and tail risk**, not per-trade win rate | **Supported, R5.** Out of sample `state_machine_v1` cuts max drawdown 43.86% → 8.24% against the R0 baseline and lifts winner/loser ratio 1.63 → 2.02, while total return stays *below* buy-and-hold (+23.3% vs +85.8%). Win rate moves only 37.7% → 42.8%. Judging on Sharpe alone would have understated it; judging on return alone would have called it a failure. |
...
| C3 | Breakouts after volatility compression outperform ordinary breakouts | **Not supported via `compression_release`, R5.** On both halves the state-machine commitments a release preceded paid *less* than the ones it did not (−0.36% vs +0.93% at 6 bars in-sample, −0.39% vs +0.33% out of sample). A firing does mark a ~12–13% larger *absolute* 6-bar move on both halves — expansion without direction — and that is gone by 30 bars. This tests one feature at one threshold, not the hypothesis in full; a breakout-conditioned version (compression percentile *at the breakout bar*) is untested and would be the fair retry. |
...
warms 2,160 bars against donchian's 40, and a shared frame start would have handed the
baseline 2,120 bars the machine cannot see. Everything below is **net of funding**, at the
engine defaults (10bp/side, `--exit-mode opposite_signal_only`, fixed sizing, 95%
deployment, 10,000 initial).

Training half: 6,910 tradeable bars, 2020-09-04 08:00 → 2023-10-30 20:00, buy-and-hold
**+230.90%**. Test half: 6,048 bars, 2023-10-31 00:00 → 2026-08-03 20:00, buy-and-hold
**+85.78%**.

| Run | half | net return | Sharpe | max DD | trades | time in market | funding |
|---|---|---|---|---|---|---|---|
| `donchian` 40/10 (R0 baseline) | train | +179.26% | **+1.433** | 17.88% | 112 | 52.1% | 2,360 |
| `state_machine_v1` (R4 default) | train | +3.56% | +0.131 | 23.11% | 286 | 32.6% | 176 |
| `state_machine_v1` (trained, best of 54) | train | +34.72% | +0.769 | 13.61% | 177 | 25.2% | 159 |
| `donchian` 40/10 (R0 baseline) | **test** | −6.64% | +0.072 | 43.86% | 114 | 51.2% | 438 |
| `state_machine_v1` (R4 default) | **test** | +19.36% | +0.738 | 11.37% | 212 | 28.8% | 6.6 |
| `state_machine_v1` (trained) | **test** | **+23.29%** | **+0.938** | **8.24%** | 159 | 24.7% | 4.4 |

The machine's near-zero funding is a balanced book, not an error: 86 long / 73 short out of
sample, paying 102.7 and receiving 98.3. Donchian pays 843.2 and receives 405.5 on a book
that is in the market twice as often at full size.

**Selected configuration** (training half, highest net-of-funding Sharpe of 54):
`StateMachine(enter_strength=0.80, exit_strength=1/3, min_dwell=2, cooldown=16)`. Marginal
means of training Sharpe show one dominant axis — `enter_strength` 0.55 / ⅔ / 0.80 →
+0.012 / +0.073 / **+0.533** — and the optimum sits on the **edge of the swept range**,
which is the shape the R0 15m gate was rejected for. The search wants a machine that trades
even less, and the limit of that is not trading. Read the selected cell as "the most
selective setting tried", not as an interior optimum.
...
|---|---|---|---|
| train (bars 2160–9069) | 16/16 | +0.829 | 40/10 at +1.433 |
...
| `state_machine_v1` (default) | +19.36% | +3.63% | −12.09% |
| `state_machine_v1` (trained) | +23.29% | +11.34% | −0.61% |

**Where the PnL comes from** — every out-of-sample trade classified by the `strength` band
its *entry* bar sat in, which is what the policy reads:

| band | half | trades | PnL | share of total | win rate |
|---|---|---|---|---|---|
| follow (top tercile) | train | 101 | +3,100.91 | +85.4% | 42.6% |
| fade (mid tercile) | train | 76 | +529.99 | +14.6% | 39.5% |
| follow (top tercile) | **test** | 94 | **+2,389.81** | **+102.4%** | 43.6% |
| fade (mid tercile) | **test** | 65 | −56.27 | −2.4% | 41.5% |
...
|---|---|---|
| firings / commitments | 697 / 55 | 666 / 53 |
| P(commit within 6b \| release) vs base rate | 6.74% vs 4.78% | 5.71% vs 5.26% |
| signed forward return, commitments **preceded** vs not, 6b | −0.36% vs +0.93% | −0.39% vs +0.33% |
| signed forward return, preceded vs not, 30b | +0.13% vs +3.72% | +0.07% vs +0.85% |
| \|6-bar move\| after a firing vs all bars | 2.69% vs 2.37% | 1.94% vs 1.73% |
| \|30-bar move\| after a firing vs all bars | 5.84% vs 5.63% | 3.99% vs 3.96% |

The only sign-stable, replicating effect is the fourth row: a firing marks a ~12–13% larger
absolute move over the next 6 bars, gone by 30. No directional content, no timing lift that
survives the split, and the transitions it precedes pay less rather than more. Not wired.
...
| M12 | A stable IC is not a tradeable one, and P&L is what settles it | R5: R4's mid-`strength` tercile had the larger absolute IC (−0.113) with *both halves agreeing*, and the high tercile was the one flagged as decaying. Out of sample the follow band produced +102.4% of the machine's PnL and the fade band −2.4%. Cross-sectional rank correlation and a traded book disagreed about which regime was real; the book is what the program is gated on. | 2026-08-04 |
| M13 | `compression_release` stays measured and unwired | R5 judged it on the bars where it fires, which R4 could not: the timing lift (6.74% vs a 4.78% base rate) does not replicate out of sample (5.71% vs 5.26%), and the transitions it precedes pay less on both halves. It marks a short-lived volatility expansion with no direction. Kept in `features/registry.py` because it is measured and cheap, wired into nothing, per §8.7. | 2026-08-04 |

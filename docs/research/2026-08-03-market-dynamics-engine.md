...
| Q6 | Does the read-only research browser get built before R9, or does R9 come first? It is small, runs on the vectorized path, and sidesteps all six live-vs-backtest divergences by construction — but it visualises numbers R9 has not yet stress-tested, and ETH already showed the edge is one scalar by 0.083 with the untuned machine failing out of sample. Build it only if the fetch → re-run → regenerate → reload loop is felt as friction today. | R7, R10 |

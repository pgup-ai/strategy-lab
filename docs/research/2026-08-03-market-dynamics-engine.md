...
([§9.4](#94-eth-replication-of-the-r5r6-protocol--ethusdt-perp-4h)); R7 (meta-model) is
next and wants re-aiming
**Owner:** Jingbo
**Created:** 2026-08-03
...
2. **The selection scalar is diluted by each cell's own warmup.** R5 scores the whole
   window including the leading flat run, and each cell's frame starts at its own warmup.
...
3. **Five of the 16 donchian cells duplicate another**, so "16/16" overstates breadth;

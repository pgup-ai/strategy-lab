...
1. **`state_machine_v1` on an equity is the M20 condition, and the tile does not flag it.**
   Measured: it answers on the four hourly equity sets with `crowding_measured=False`,
   `funding_attached=False` and `crowding` pinned at 0.5 on every bar — `with_funding_column`
   returns early for a non-perp, so `required=True` is never consulted and the machine runs
   on **three** of its four features. On a perp the same run would be *refused*. The
   instrument view's provenance strip does state `crowding_measured no`, but the **board
   tile's** warning is gated on `market_type === 'perp'` and so says nothing. It is left
   alone here because the honest predicate is *"does this strategy read a funding-derived
   feature"* rather than the market type — un-gating on market type alone would put a
   spurious warning on every `donchian` tile — and that is a change to what a perp tile says,
   which check 6 forbids in this phase. Carried to §12.
...
| The board tile's `crowding_measured` warning is gated on `market_type === 'perp'` | Found by R10c ([§9.12](#912-r10c-equities-on-the-board--census-item-f-and-what-a-tile-can-honestly-claim)). `state_machine_v1` on an equity runs on three of its four features with `crowding` pinned at 0.5 — the M20 condition, permanently, where a perp would be *refused* — and the tile is silent. The instrument view's provenance strip does state it. The fix is a predicate on **whether the strategy reads a funding-derived feature**, not on the market type, since un-gating on market type alone puts a spurious warning on every `donchian` tile |

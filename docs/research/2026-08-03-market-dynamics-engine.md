...
**Status:** R0–R6 complete; R7 (meta-model) is next
**Owner:** Jingbo
**Created:** 2026-08-03
...
| R5 | Rule-based state machine | Beats R0 baseline out-of-sample, with hysteresis + dwell time + cooldown | R4 | **Done — gate passes**, re-measured after the bounded-exit fix (see the 2026-08-04 correction in §9). OOS Sharpe **+0.896** against the baseline's +0.072, max drawdown **4.67%** against 43.86%; the untuned default also passes at +0.746. 54 configurations on the training half, and the selection is now re-derived by test rather than pinned. **Wins on risk, not return** (+15.5% against buy-and-hold's +85.8%), loses to the test-half oracle donchian cell, but **now survives 3× costs** (+6.4%). The result is the follow band, not the mid-tercile fade. See [§9.2](#92-r5-split-sample-gate--btcusdt-perp-4h). |
| R6 | Continuous-exposure contract | Second strategy contract; vectorbt replaced for this path | R5 | **Done — gate passes.** `TargetExposure` executes through `from_orders`, has its own determinism and lookahead proofs, and the four original strategies stay byte-identical. Retiring vectorbt was not required and is not in scope (Q4). **The taper measures ≈0** (+15.30 / −63.05 gross over 75/77 bars); what v2 buys is 1.7–1.8× v1's average exposure and the return scales with it, while Sharpe is flat. The finding is that `state_machine_v1` never sized above §2.4's **BREAKOUT** row — see [§9.3](#93-r6-continuous-exposure-comparison--btcusdt-perp-4h). |
| R7 | Meta-model (logistic) | Improves position sizing, not direction | R6 | — **wants re-aiming before it starts**: R6 measured the earning difference in the *ramp* (holding more while a move is establishing), not the taper, and no per-state effect on the test half is distinguishable from zero (no t-stat above 1.77). A sizing model aimed at trimming mature trends would be aimed at the half that measured nothing. |
...
**Read these figures as a machine sizing at its `BREAKOUT` row.** Nothing here is
...
`RIDING`. The lifecycle is strictly forward and `from_signals` reads size at entry, so the
entry bar is always the smallest non-zero row of §2.4's table and the position is frozen
there. The +15.45% below was earned at roughly a third of the risk the policy asked for.
See [§9.3](#93-r6-continuous-exposure-comparison--btcusdt-perp-4h).
...
**Why: v1 never sizes above `BREAKOUT`.** Of v1's size-setting entry bars, **not one is in
`RIDING`**, in either configuration — trained 56 `BREAKOUT` (target 0.35) + 15 `EXHAUSTION`
(0.55); default 92 `BREAKOUT` + 20 `CONFIRMED` (0.70) + 40 `EXHAUSTION`. So the largest
position v1 ever opened is **52.3% of capital (trained), 66.5% (default)** against the
**95%** a `RIDING` target asks for, while the machine spent **209 / 516 bars in `RIDING`**.
The lifecycle is strictly forward, so the bar on which the target first goes non-zero is
always the *smallest* non-zero row of §2.4's table; `from_signals` reads size at entry only,
so the position is frozen there and cannot scale when the move is confirmed. **§2.4's table
has never executed above its `BREAKOUT` row.** Relative to a position frozen at the lowest
level, nearly every later state is a step *up* — which is why the "taper" measures as a
ramp. *(A separate check using a cruder funding alignment counted 71/152 entries rather
than 73/153 and 56/92 `BREAKOUT`; the structural claim — zero `RIDING` entries, largest
size 0.55 / 0.70 — reproduced exactly.)*
...
finding is not about the taper at all — it is that `state_machine_v1` has never executed
the upper three rows of its own policy table, and R5's published +15.45% was earned at
...
| M16 | The taper measures zero; the *ramp* is what earns, and §2.4's upper rows have never executed | R6 on R5's test half. v2 held more than v1 on 523 / 807 bars for **+1,445.31 / +2,783.64** gross and less on 75 / 77 bars for **+15.30 / −63.05**. The cause is structural: not one of v1's size-setting entry bars is in `RIDING` in either configuration, so the largest position it ever opened is 52.3% / 66.5% of capital against the 95% a `RIDING` target asks for, on a machine that spent 209 / 516 bars there. The lifecycle is strictly forward and `from_signals` reads size at entry, so the entry bar is always the smallest non-zero row of the table and the position freezes there — the "taper" therefore measures as a ramp. Sizing research after this (R7) aims at the ramp; the taper is measured and not worth a rule. | 2026-08-04 |

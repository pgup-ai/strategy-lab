...
Aggregating the same returns to daily (≈1,119 days, excess kurtosis +29.2) lifts the DSR to
**≈0.77**, still short of 0.95. The frequency was **not fixed by the plan** and the daily
aggregation sits outside the committed pipeline, so its exact value moves with the choice of
day boundary — which is the point, and is why M23 requires a declared frequency next time.
...
| Q7 | **R9 measured re-selection losing to holding a pre-committed cell (M22). What does R7 become?** Three readings, and they lead to different work. **(a) Drop R7 and go to R10** — the sizing question has now returned null three times (R6's per-state effects, ETH's asset-specificity, R9's re-fit cost), and the unbuilt thing with the clearest value is the live path. **(b) Re-aim R7 at the ramp with no per-period re-fit** — a fixed sizing rule fitted once and held, which is what M22 says the machine's parameters should be, though "fitted once on the training half" is precisely the procedure R9 could not distinguish from luck. **(c) Attack the actual thesis** — the program exists to tell chop from trend, and R9 says the answer is not in the four axes: `enter_strength` is the only stable one and it is a ridge, so more grid is not more signal. This is the first open question whose answer changes the roadmap rather than a phase's contents. | R7 |
